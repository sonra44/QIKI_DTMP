...
## Сбор контекста
- [Факт] Проверяет наличие Python и библиотеки `grpc`.
- [Факт] При отсутствии сгенерированных protobuf‑файлов запускает `grpc_tools.protoc`.
- [Факт] Выполняет проверку доступности `Q-Sim` сервера через gRPC‑запрос `HealthCheck`.
- [Факт] Запускает `services/q_core_agent/main.py --grpc`.
