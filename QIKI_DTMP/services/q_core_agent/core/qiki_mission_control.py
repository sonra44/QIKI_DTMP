...
class QIKIMissionControl:
    ...
    def __init__(self):
        ...
        self.sensor_data = {
            ...
            'lidar': {'mode': 'Focused Beam', 'distance': 32.17, 'composition': 'Scanning...'},
    ...
    def _update_live_parameters(self):
        ...
        try:
            ...
            self.sensor_data['radar']['objects'] = max(5, min(10, 
                self.sensor_data['radar']['objects'] + random.randint(-1, 1)))
            
            # Прогресс миссии
            if self.mission_data['progress'] < 100:
                self.mission_data['progress'] += random.uniform(0.1, 0.3)
                self.mission_data['eta_seconds'] = max(0, 
                    self.mission_data['eta_seconds'] - random.uniform(10, 30))
    ...
    def display_status(self):
        ...
        print(f"║ 🛰️ {self.ship_core.get_id():<25} {mission_time:>15} {datetime.now().strftime('%H:%M:%S UTC'):>15} {'':>35} ║")
        print("╠" + "═" * 98 + "╣")
    ...
    def _display_navigation_panel(self):
        ...
        print(f"║   Скорость / Velocity: {vel['absolute']:4.0f} м/с абсолютная, {vel['relative']:+3.0f} м/с к цели          ║")
        print(f"║   Расстояние до {self.navigation_data['target_name']}: {self.navigation_data['anomaly_distance']:6.2f} км               ║")
        print(f"║   Сектор / Sector: {self.navigation_data['sector']:<20} Дрейф / Drift: {vel['drift']:.2f}%/ч         ║")
    ...
    def _display_mission_panel(self):
        ...
        lang_title = "УПРАВЛЕНИЕ МИССИЕЙ / MISSION CONTROL" if self.language == "RU" else "MISSION CONTROL / УПРАВЛЕНИЕ МИССИЕЙ"
        lang_progress = "Прогресс / Progress" if self.language == "RU" else "Progress / Прогресс"
        lang_eta = f"ETC: {eta_hours:02}ч {eta_mins:02}м" if self.language == "RU" else f"ETC: {eta_hours:02}h {eta_mins:02}m"
        ...
        print(f"║   {lang_eta:<20} Этапы / Steps завершено: {sum(1 for s in mission['steps'] if s['done'])}/{len(mission['steps'])}              ║")
    ...
    def _get_alert_level(self, telemetry: Dict[str, Any]) -> str:
        ...
        if (telemetry['hull_integrity'] < 30 or
            telemetry['oxygen_level'] < 16 or
            telemetry['reactor_temp'] > 3500):
            return "🚨 EMERGENCY"
        elif (telemetry['hull_integrity'] < 60 or
              telemetry['battery_charge'] / max(telemetry['battery_capacity'], 1) < 0.2 or
              telemetry['main_drive_fuel'] < 50):
            return "⚠️  WARNING"
        elif (telemetry['hull_integrity'] < 80 or
              telemetry['co2_level'] > 1000 or
              telemetry['reactor_temp'] > 3000):
            return "⚡ CAUTION"
        else:
            return "✅ NOMINAL"
        ...
        try:
            ...
            # === RCS КОМАНДЫ ===
                ...
                if len(cmd_parts) >= 3:
                    ...
                    axis_map = {
                        "FORWARD": ThrusterAxis.FORWARD, "FWD": ThrusterAxis.FORWARD,
                        "BACKWARD": ThrusterAxis.BACKWARD, "AFT": ThrusterAxis.BACKWARD,
                        "PORT": ThrusterAxis.PORT, "LEFT": ThrusterAxis.PORT,
                        "STARBOARD": ThrusterAxis.STARBOARD, "RIGHT": ThrusterAxis.STARBOARD
                    }
                    ...
                    self.log("ERROR", "❌ Usage: rcs <forward|aft|port|starboard> <0-100> [duration]")
            ...
            # === УПРАВЛЕНИЕ ПИТАНИЕМ ===
                ...
                if len(cmd_parts) >= 2:
                    ...
                    if subcmd == "status":
                        power = self.ship_core.get_power_status()
                        self.log("POWER", f"⚡ Reactor: {power.reactor_output_mw:.1f}/{power.reactor_max_output_mw:.1f} MW")
                        self.log("POWER", f"🔋 Battery: {power.battery_charge_mwh:.1f}/{power.battery_capacity_mwh:.1f} MWh")
                        self.log("POWER", f"🌡️ Temp: {power.reactor_temperature_k:.0f}K")
                        return True
                    elif subcmd == "emergency":
                        # Аварийное отключение неосновных систем
                        allocation = PowerAllocation(
                            life_support=10.0, propulsion=5.0, sensors=2.0, 
                            qiki_core=3.0, shields=0.0
                        )
            ...
            # === СТАТУС СИСТЕМ ===
                ...
                self.log("STATUS", f"🧭 Navigation: {'READY' if summary.get('navigation_capable') else 'LIMITED'}")
                return True
            ...
            # === ДИАГНОСТИКА ===
                ...
                self.log("DIAGNOSTICS", f"⚡ Reactor: {health.get('reactor_output_percent', 0):.1f}%")
                self.log("DIAGNOSTICS", f"🔋 Battery: {health.get('battery_charge_percent', 0):.1f}%")
    ...
    try:
        ...
        def create_live_interface():
            ...
            def toggle_lang(event):
                ...
                mission_control.log("INTERFACE", f"🌐 Language: {'English' if mission_control.language == 'EN' else 'Русский'}")
                status_area.text = mission_control._create_status_content()
