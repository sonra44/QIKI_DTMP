...
class ShipActuatorController:
    ...
    def fire_rcs_thruster(self, thruster_axis: ThrusterAxis, thrust_percent: float, duration_sec: float = 1.0) -> bool:
        """
    ...
    def execute_maneuver(self, thrust_vector: ThrustVector, duration_sec: float = 2.0) -> bool:
        ...
        try:
            ...
            if abs(thrust_vector.y) > 0.1:
                total_commands += 1
                axis = ThrusterAxis.STARBOARD if thrust_vector.y > 0 else ThrusterAxis.PORT
    ...
    def emergency_stop(self) -> bool:
        ...
        try:
            ...
            for axis in [ThrusterAxis.FORWARD, ThrusterAxis.BACKWARD, ThrusterAxis.PORT, ThrusterAxis.STARBOARD]:
    ...
    def set_power_allocation(self, allocation: PowerAllocation) -> bool:
        ...
        try:
            ...
            total_requested = (allocation.life_support + allocation.propulsion + 
                             allocation.sensors + allocation.qiki_core + allocation.shields)
            
            if total_requested > power_status.reactor_max_output_mw:
                logger.warning(f"Power allocation exceeds reactor capacity: {total_requested:.1f}/{power_status.reactor_max_output_mw:.1f} MW")
                return False
    ...
    def get_control_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'systems_controllable': [
                'main_drive', 'rcs_thrusters', 'power_distribution', 
                'sensor_array', 'life_support'
            ]
        }
