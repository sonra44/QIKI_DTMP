...
try:
    ...
    from .ship_core import ShipCore, HullStatus, PowerSystemStatus, PropulsionStatus, SensorStatus, LifeSupportStatus, ComputingStatus
except ImportError:
...
class ShipBiosHandler(IBiosHandler):
    ...
    def process_bios_status(self, bios_status: BiosStatusReport) -> BiosStatusReport:
        ...
        logger.info(f"Ship diagnostics complete. Systems: {'NOMINAL' if all_systems_go else 'DEGRADED'}, "
    ...
    def _diagnose_power_systems(self) -> tuple[PowerSystemStatus, List[Dict[str, str]]]:
        ...
        # Check reactor
        reactor_efficiency = power_status.reactor_output_mw / power_status.reactor_max_output_mw if power_status.reactor_max_output_mw > 0 else 0
        ...
        # Check batteries
        battery_charge = power_status.battery_charge_mwh / power_status.battery_capacity_mwh if power_status.battery_capacity_mwh > 0 else 0
    ...
    def get_system_health_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'reactor_output_percent': (power.reactor_output_mw / power.reactor_max_output_mw * 100) if power.reactor_max_output_mw > 0 else 0,
                'battery_charge_percent': (power.battery_charge_mwh / power.battery_capacity_mwh * 100) if power.battery_capacity_mwh > 0 else 0,
