...
import os
import logging
from .core.generator import create_design_document, create_proto_contract, compile_protos, build_readme
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Выполнить в режиме симуляции без фактических изменений файлов.")
    parser.add_argument("--template", type=str, default="default", help="Имя шаблона для использования (например, 'default', 'minimal').")

    subparsers = parser.add_subparsers(dest="command", help="Доступные команды", required=True)

    # --- Команда 'new' ---
    parser_new = subparsers.add_parser("new", help="Создать новый компонент (design.md + .proto).")
    parser_new.add_argument("name", type=str, help="Имя компонента (например, 'Q-Sim-Service').")
    parser_new.add_argument("--force", action="store_true", help="Перезаписать файлы, если они существуют.")

    # --- Команда 'compile-protos' ---
    parser_compile = subparsers.add_parser("compile-protos", help="Скомпилировать все .proto контракты.")

    # --- Команда 'build-readme' ---
    parser_readme = subparsers.add_parser("build-readme", help="Собрать главный README.md из документов.")
    ...
    try:
        if args.command == "new":
            ...
            if not args.dry_run:
                ...
                logger.info(f"Dry Run: Директория компонента будет создана: {component_dir}")
            
            target_file_md = os.path.join(component_dir, f"{args.name}_design.md")
            create_design_document(args.name, target_file_md, args.force, args.dry_run, args.template)
