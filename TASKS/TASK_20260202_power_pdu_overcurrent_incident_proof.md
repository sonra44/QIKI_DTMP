...
**Date:** 2026-02-02  
**Goal:** Prove the operator loop for a Power Plane PDU overcurrent condition: sim event → ORION incident → operator `ack` + `clear` → incident lifecycle audit events.
...
Fetch from JetStream `QIKI_EVENTS_V1`, subject `qiki.events.v1.operator.incidents` and verify:
...
- Incident lifecycle audit events (JetStream `QIKI_EVENTS_V1`, subject `qiki.events.v1.operator.incidents`):
