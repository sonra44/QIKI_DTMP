...
2) Verifies the capture contains at least one deterministic `overcurrent=1` payload (the test injects one into the record window).
...
- Test passes deterministically in canonical Docker run (no fixture-dependent skip branch).
