...
**Date:** 2026-02-02  
**Goal:** Prove the operator loop for thermal incidents: sim event → ORION incident → operator `ack` → operator `clear` → incident lifecycle audit events.  
...
3) ORION incident lifecycle proof (tmux/attach):
