...
**Date:** 2026-02-02  
**Goal:** Prove the operator loop for a thermal TRIP (edge event): trip → event → ORION incident → operator `ack` + `clear` → incident lifecycle audit events.  
...
- ORION commands (in input):
  - `ack TEMP_CORE_TRIP|sensor|thermal|core`
  - `clear`
- Observed incident lifecycle audit events (JetStream `QIKI_EVENTS_V1`, subject `qiki.events.v1.operator.incidents`):
