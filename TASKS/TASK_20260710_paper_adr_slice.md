...
Запрос оператора: «проверка, поиск ошибок + интернет + исправление».
Два адверсариальных субагента (ADR↔код, ADR↔канон) + внешняя web-сверка.

Подтверждено: все file:line-якоря (пороги матчеров, N7-путь мимо
WorldModel, compose:204/247, отсутствие queue-group с обеих сторон,
подписка ORION V на мостовые треки, схлопывание кадра — единственный
путь публикации моста); внешняя практика совпадает по всем трём
решениям (e-stop минует диалог но не interlock+аудит; centralized
fusion vs track-to-track; dual-master problem → один master).

Найдено и исправлено (коммит этого дополнения):
1. Репо-копия `CONTEXT_LOCK` п.5 не содержала клаузы «dual-source
   запрещён без явного ADR» (была только в ~/MEMORI с 2026-03-01) —
   репо-копия синхронизирована; оба ADR цитируют точный путь.
2. `SENSOR_OBS_0001_…md` отсутствовал в репо (doc-часть патча не
   доехала, код доехал) — документ восстановлен в
   `docs/design/sensor_observation/` из `ПАЧИ/999999/…patch`.
3. BRAKE ADR цитировал дорешенческую редакцию G4 — обновлён под
   HEAD-текст (3b5eb7f); восстановлена вырезанная часть цитаты §12;
   явно развязано «не игровая кнопка» (REQ-RCS-005, про сопла) vs
   override-жест; проговорено расхождение с лором («всегда разрешён»
   → подчинено gating); честная оговорка о происхождении триггера
   (REPLY_002 требовал ADR после Блока 0; ранний ход санкционирован
   id=5161/5163); уточнено «confirmation ≠ второе действие» (§18.4/18.6);
   ссылка propose/commit → `F5_QIKI_DIALOG_SYSTEM_DESIGN.md` §3.
4. Identity-ADR: «транслятор» N8 → честное «двухключевой carry с
   fallback-резолвом, public-ветка в кросс-домене мертва» (S1-мини
   обязан её починить); «спуф by design» помечен интерпретацией;
   добавлен пятый путь (SR-кадры q-sim → legacy-консоль, вне скоупа).
5. Тик-ADR: снят устаревший аргумент «дефект 0.8» (на HEAD уже
   `asyncio.to_thread`) — альтернатива 1 отклоняется живыми доводами;
   исправлено «после ADR идентичности» → «после K2-кода».
