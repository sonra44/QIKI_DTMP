...
class TestFsmSnapshotDTO:
    ...
    def test_add_transition_history_limit(self):
        ...
        snap = snap.add_transition(
            create_transition(FsmState.IDLE, FsmState.ACTIVE, "E1"), max_history=2
        )
        snap = snap.add_transition(
            create_transition(FsmState.ACTIVE, FsmState.ERROR_STATE, "E2"),
            max_history=2,
        )
        snap = snap.add_transition(
            create_transition(FsmState.ERROR_STATE, FsmState.IDLE, "E3"), max_history=2
        )
