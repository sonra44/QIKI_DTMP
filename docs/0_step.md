...
Registrar = audit/support fan-in/fan-out service: читает выбранные subjects, пишет локальный append-only audit log и публикует registrar-owned audit copies в `qiki.events.v1.audit`.
Не является owner runtime truth, не является owner intent/control и не является единственным writer для `qiki.events.v1.audit`.
