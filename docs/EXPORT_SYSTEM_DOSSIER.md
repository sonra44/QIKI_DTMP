...
Terminology note:
- `ORION Shell OS` in this dossier means the canonical ORION V operator surface.
- standalone service `shell_os` is a separate diagnostic/support overlay and not an operator canon path.
...
- `qiki-operator-console` — ORION V console (`src/qiki/services/operator_console/main_orion_v.py`), legacy ORION retained for compatibility/reference
