...
- Еженедельный обзор: пересмотр приоритетов/сроков по канону `~/MEMORI/ACTIVE_TASKS_QIKI_DTMP.md`, зависимости фиксировать в task‑досье `TASKS/`.

### 2.4 Дорожные карты и зависимости
- Поддерживать `IMPLEMENTATION_ROADMAP.md`, `STEP_A_ROADMAP.md`; связи и риски — в task‑досье `TASKS/` (напр., `P3-001` зависит от `P3-002`).
...
## 6. Документация
- Обновлять `README.md`, `RADAR.md` и соответствующие task‑досье в `TASKS/` при изменениях API/архитектуры; канон приоритетов — `~/MEMORI/ACTIVE_TASKS_QIKI_DTMP.md`.
