...
> NOTE (2026-01): current ORION console exists and evolved beyond this early plan.
> For ongoing TUI work use the specialized agent: `docs/agents/orion_tui_agent.md`.
...
1. **Анализ требований и подготовка среды**
   - Изучите текущие документы проекта (`README.md`, `RADAR.md`, `bot_gdd.md`, `TASKS/` и канон `~/MEMORI/ACTIVE_TASKS_QIKI_DTMP.md`) и выделите данные, которые должны отображаться в консоли: состояние FSM, треки радара и метрики JetStream, события Guard Rules, параметры аллокатора тяги, состояние батарей и другие сервисы.
...
2. **Настройка Docker и CI**
   ...
   - Обновите task‑досье в `TASKS/` и ссылку в каноне `~/MEMORI/ACTIVE_TASKS_QIKI_DTMP.md`, описав задачи по разработке консоли.
