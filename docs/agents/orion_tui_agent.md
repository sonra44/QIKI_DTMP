...
# Контекст проекта
- ORION V: `src/qiki/services/operator_console/main_orion_v.py` (supported Textual runtime).
- Legacy ORION reference entrypoint: `src/qiki/services/operator_console/main_orion.py`.
...
- Живой ORION V под `tmux`: `./scripts/run_orion_v_live.sh`
- Source of truth внутри helper: `docker exec -it qiki-operator-console python main_orion_v.py`
- `docker-compose.operator_orionv.yml` считать transitional/non-canonical override, а не обычным стартовым путём.
- Не использовать `docker attach qiki-operator-console` как стандартный live-path для ORION V под `tmux`.
