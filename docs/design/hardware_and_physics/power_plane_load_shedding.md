...
ORION visibility (M1 acceptance):
- ORION legacy power table renders `power.shed_reasons` as a dedicated row (no fake defaults).
- ORION V power module renders:
  - summary: `Причины сброса ...` from `power.shed_reasons`,
  - details: `Аварийное отключение нагрузки` + `Причины сброса`,
  - degraded marker (`degraded: нет данных`) when shedding is active but reasons are missing.
