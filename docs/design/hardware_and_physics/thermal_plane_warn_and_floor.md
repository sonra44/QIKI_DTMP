...
- `t_warn_c` override имеет приоритет над derived `trip - warn_delta`,
- `warned` выставляется до `tripped`,
- trip clear соблюдает гистерезис и не флапает на boundary `trip-hys`,
- нет overshoot ниже ambient из состояния выше ambient,
- если узел уже ниже ambient — клэмпа вверх нет (ниже ambient допустимо в текущей модели).
