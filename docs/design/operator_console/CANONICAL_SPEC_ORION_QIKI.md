...
- **Clickable cockpit:** `F1` must expose direct clickable transitions from summary sections to detailed screens, plus explicit clickable controls for pending QIKI confirmations/cancellations when such actions exist.
- **Contextual actions only:** navigation may stay globally visible, but screen-specific actions must hide or disable themselves outside the context where they make operational sense.
