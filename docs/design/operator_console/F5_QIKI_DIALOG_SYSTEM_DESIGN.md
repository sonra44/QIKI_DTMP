...
3. ufw — **ПРОВЕРЕНО 2026-07-05 (sudo, живьём)**: ufw НЕ установлен. Факты:
   наружу слушает ТОЛЬКО sshd (0.0.0.0:22) под fail2ban (f2b-sshd REJECT);
   ВСЕ Docker-DNAT (QIKI NATS/gateway, весь RAGFlow-стек) — строго на
   127.0.0.1 (published-порты проверены через `iptables -t nat`, не только
   `ss`); INPUT policy ACCEPT. Вердикт: критичной экспозиции нет, для
   прототипа достаточно. Опция второго рубежа (решение оператора, отдельно):
   ufw «allow OpenSSH, deny incoming» — помнить docker-caveat (контейнерные
   publish обходят ufw через FORWARD; страховка от будущих 0.0.0.0-publish —
   правило в DOCKER-USER, не ufw).
4. ADR на enum'ы `runtime_claim_status`/`source_type` — завести до M4/M5.
5. Строка №9 ACTION RAIL — **ВЫПОЛНЕНО 2026-07-05** (решение оператора «сначала
   косметика и техдолг»): show-when инцидент-кнопок, CMD-подсказка без перечня,
   кнопка «F5 QIKI» (долг M1), снос мёртвого `_action_rail_hint`.
   Реестр: DISPLAY_CANON §4 строка №9; live-proof: F5 открыт из консоли.
...
- **M7-M9 мост ПОДКЛЮЧЁН к живому контуру (2026-07-05, ADR-0018)**: intent
  «установи модуль» → живой policy (BODY_ATTACH-кандидат) → approve (M6) →
  пломба/authorize (M5) → `bridge_decision_to_body` → ЖИВОЙ
  `BodyStructureInteractiveController` (тот же конвейер, что клавиша `b`) →
  трасса в реальный EVENTS_AUDIT (`runtime_claim_status` по ADR-0017).
  Предусловия fail-closed из готовых VM (power: load_shedding/pdu_throttled/нет
  телеметрии; thermal: статус ≠ green). Live-proof:
  `scripts/prove_orion_v_qiki_body_attach_live.sh` (полный цикл по реальной шине).
...
**Геймплейный долг — ЗАКРЫТ 2026-07-05:**
- B1: оператор видит РЕАЛЬНУЮ команду (subject/name/params) — fc0bf18;
- B2: мост M7-M9 подключён к живому пути (ADR-0018, см. выше).
Остаточный хвост: клавиши `/`/`:` не доходят до app через tui-test PTY
(F-клавиши доходят) — ограничение драйвера/фокуса, живой оператор не задет.

**Развитие 2026-07-05/06 (после закрытия долга):**
- ADR-0019 (P1-P4): бортовой каталог модулей — параметризованная установка
  («установи <модуль> на <гнездо>»), quantity-дефицит, полный паспорт в пломбе.
- ADR-0020 (P0-P4): установка = многошаговая процедура (S1 осмотр → S2
  подготовка → S3 перенос во времени → S4 питание → S5 стыковка) с
  осложнениями-развилками только из реального состояния; процедура владеет
  захваченным решением; кнопка ⏸/▶ на рейле; строка УСТАНОВКА в зоне РЕШЕНИЕ
  с условием допустимости (G1); лента «ПРОЦЕДУРА ▸» (не голос QIKI).
  Prove: `scripts/prove_orion_v_attach_procedure.sh`.
