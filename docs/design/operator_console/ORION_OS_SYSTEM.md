...
ORION is the **Operator Console TUI** (“Shell OS”) built on **Textual**. It is a stable cockpit “chrome” (header/sidebar/inspector/temporary command mode) with multiple screens that all follow the same interaction model:
...
In Phase1 the canonical operator console is launched via:

- `docker compose -f docker-compose.phase1.yml -f docker-compose.operator.yml up -d --build operator-console`

Key services involved:

- **`qiki-operator-console`**: runs ORION V (`src/qiki/services/operator_console/main_orion_v.py`).
...
Launch canon:
- supported overlay: `docker-compose.operator.yml`
- canonical live TTY path under `tmux`: `./scripts/run_orion_v_live.sh`
- transitional override: `docker-compose.operator_orionv.yml` (kept only for compatibility with older proofs)
- legacy reference entrypoint: `src/qiki/services/operator_console/main_orion.py` (not the default runtime path)
...
Notes:
- Current ORION V screen composition lives in `src/qiki/services/operator_console/orion_v/app.py`.
...
- ORION V runtime/UI composition: `src/qiki/services/operator_console/orion_v/app.py`
