...
- ✅ Sidebar: хоткеи и пункты меню не режутся `…` в tmux — sidebar использует короткие RU-лейблы без сокращений (например `Ctrl+P Двигатели`).


## Update (2026-01-18) — ORION Keybar fit (tmux-safe)

- ✅ Keybar больше не режет хоткеи посередине (без `Ctrl+N Sensors/Сенс…`).
- ✅ Формат: разделители `·`, активный экран подсвечен префиксом `▶`.
- ✅ `F9 Help/Помощь` и `F10 Quit/Выход` всегда видимы: при нехватке ширины выкидываются менее важные пункты меню, а не «ломаются» токены.
- ✅ Проверено в tmux (attach к `qiki-operator-console`).
