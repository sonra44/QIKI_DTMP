...
- Docking команды (`sim.dock.*`, `power.dock.*`) исполняются только в `FACTORY/ЗАВОД`; в `MISSION/МИССИЯ` блокируются с предупреждением и audit‑событием.
...
- Канон сим-управления и семантики: `docs/design/operator_console/SIMULATION_CONTROL_CONTRACT.md`.
- В ORION есть команды: `simulation.start [speed]/симуляция.старт [скорость]`, `simulation.pause/симуляция.пауза`, `simulation.stop/симуляция.стоп`, `simulation.reset/симуляция.сброс`.
  - `simulation.reset` требует ConfirmDialog (Y/N) и по решению канона означает `reset = stop + clear world`.
- `q_sim_service` принимает `sim.start/sim.pause/sim.stop/sim.reset` через `qiki.commands.control` и управляет фактическим тикающим циклом.
  - RUNNING: `world_model.step()` + radar publish.
  - PAUSED/STOPPED: мир не тикает; radar publish остановлен; telemetry продолжает идти.

### 6.2 Реальная семантика active pause (сейчас)

Текущее состояние является “реальным active pause”:

- runtime state хранится в `q_sim_service` и публикуется наружу как `telemetry.sim_state` (extra ключ).
- `sim.pause`/`sim.stop` останавливают публикацию radar frames.
- `sim.start` принимает опциональный `speed` (множитель тика).
