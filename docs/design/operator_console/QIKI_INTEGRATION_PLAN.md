...
## 2) Контракт “Intent → Proposals → (approve) → Commands”
...
MVP рекомендует **request/reply** (одна точка входа) вместо “вечных топиков”:
- subject: `qiki.chat.v1` (см. канонический контракт)

Публичные стримы `qiki.intent.v1` / `qiki.proposals.v1` допускаются позже для аудита/телеметрии, но не нужны для первого запуска.
...
### 2.3 Approve/Execute (позже)

Только после явного подтверждения оператором QIKI формирует/отправляет `ActuatorCommand` на control bus:
...
1) ORION: добавить режим `QIKI INPUT/ВВОД QIKI` и публикацию Intent.
...
4) Только потом: approve → команды/исполнение.
...
- В текущем Phase1 обработчик control команд (`faststream_bridge`) **только подтверждает получение** и **не управляет** фактическим циклом симуляции.
- `q_sim_service` сейчас тикает по таймеру (`step()` + `sleep(sim_tick_interval)`) и не имеет состояния `paused/time_scale`.
