...
| 0.3 | Гонка `q abort`: `_advance_attach_procedure`/`_attach_procedure_dock` не перепроверяют `proc.status` после await → модуль ставится ПОСЛЕ «прервано», ABORTED перезаписывается | `src/qiki/services/operator_console/orion_v/app.py:3530-3744` | ломает Consequence Confirmation (P3) | guard `proc.status` после каждого await. **Закрыт этапом 1** (task-0035, PR #3) |
| 0.4 | ~25 голых `asyncio.create_task` (вкл. execute подтверждённого действия, `app.py:3160`) — GC может убить таск, исключения теряются | `app.py`, по всему файлу | молчаливая потеря исполнения (P3, P5) | `_bg_tasks` set + done_callback с логом. **Закрыт этапом 1 полностью** (27/27 конвертированы, task-0035, PR #3; REPLY_003 A10) |
| 0.5 | TOCTOU пломбы: shallow-копия parameters при seal; мутация вложенных структур между authorize и effect не ловится | `src/qiki/shared/command_decision.py:92`; `app.py:3176-3200`; `src/qiki/shared/decision_body_bridge.py` | подрывает «одобрено = исполнено» (P3) | deepcopy при seal + повторная сверка digest перед эффектом в bridge (`BRIDGE_SEAL_DIGEST_DRIFT`). **Закрыт этапом 1** (task-0035, PR #3) |
