# Дизайн: BIOS (Basic Input/Output System)

## 0. Статус реализации (важно)

Этот документ описывает **целевой дизайн** BIOS. Текущая реализация `q-bios-service` — **MVP** и не реализует весь список функций ниже.

`q-bios-service` в текущем runtime надо читать как **support-tier derived status layer**:
- он строит производный BIOS/status payload из `bot_config.json` + health-check `q-sim-service`;
- он **не является owner physical truth** и **не является owner intents/policy**;
- он публикует и отдаёт support/status projection для потребителей вроде ORION, не меняя ownership других слоёв.

### 0.1 Что реализовано сейчас (факт кода)

- Источник конфигурации/“железа”: `bot_config.json` (см. `src/qiki/services/q_bios_service/config.py`, `bios_engine.py`).
- HTTP API (реально): `GET /healthz`, `GET /bios/status`, `GET /bios/component/<id>`, `POST /bios/reload` (см. `src/qiki/services/q_bios_service/handlers.py`).
- Публикация в NATS (реально): subject по умолчанию `qiki.events.v1.bios_status` (см. `BiosConfig.nats_subject`).
- Канонический контракт события BIOS (реально, v1): `schemas/asyncapi/qiki.events.v1.bios_status/v1/` (JSON Schema + README).
- Startup behavior (реально): HTTP сервер поднимается сразу, publisher-loop стартует только если `BIOS_PUBLISH_ENABLED=1`, а первый payload вычисляется on-demand при первом HTTP запросе `/bios/status` или при первой publish-итерации.
- Reload behavior (реально): `POST /bios/reload` не вводит новый ownership и не делает hot-reconfigure API; в MVP это только invalidation кэша последнего derived payload, после чего следующий read/publish пересчитывает статус из текущего `BOT_CONFIG_PATH`.
- Config defaults (реально): compose/код по умолчанию используют `BIOS_LISTEN_PORT=8080`, `NATS_EVENT_SUBJECT=qiki.events.v1.bios_status`, `BIOS_PUBLISH_ENABLED=1`, `BIOS_PUBLISH_INTERVAL_SEC=5.0`.

### 0.2 Что описано здесь как намерение (ещё не реализовано)

- Чтение `bot_physical_specs.json` как аппаратного контракта.
- Расширенный API (`get_component_status`, `soft_reboot`, `hot_reload_config`, `read_port`, `write_port`) и события `hardware_ready/hardware_error`.
  - Важно: в v1 эти subject’ы **не используются**. Канон для статуса BIOS в v1 — только `qiki.events.v1.bios_status`.

## 1. Обзор
Этот документ описывает компонент BIOS, низкоуровневую "прошивку" для QIKI платформы. В архитектуре Digital Twin, BIOS выполняет ту же роль, что и в PC: он инициализирует и тестирует "оборудование" (`bot_physical_specs`) и передает управление высокоуровневой логике (`Q-Core Agent`).

**Архитектурное решение:** BIOS реализуется как отдельный, легковесный микросервис `q-bios-service` для изоляции и независимого тестирования.
В текущем MVP это отдельный сервис поддержки/диагностики, а не второй runtime и не второй owner hardware truth.

## 2. Последовательность Загрузки (Boot Sequence)
1.  **[0.00s] Подача Питания:** `q-bios-service` запускается.
2.  **[0.01s] Чтение Спецификаций:** (цель) BIOS читает `bot_physical_specs.json` для получения списка оборудования и его контрактов. (факт MVP: сейчас BIOS читает `bot_config.json`).
3.  **[0.02s] Начало POST (Power-On Self-Test):** BIOS начинает опрос каждого "порта", указанного в спецификации.
4.  **[0.02s - 0.20s] Процесс POST:** Для каждого компонента выполняется базовый тест (например, проверка ответа, запрос статуса).
5.  **[0.21s] Формирование Отчета:** BIOS генерирует `bios_status.json` с результатами POST.
6.  **[0.22s] Проверка Статуса:**
    *   **Если `status: "ready"`:** (цель) BIOS публикует `hardware_ready` и завершает активную фазу. (факт MVP: публикуется периодический `qiki.events.v1.bios_status`).
    *   **Если `status: "error"`:** (цель) BIOS публикует `hardware_error` и переходит в диагностику. (факт MVP: статус отражается внутри payload `bios_status`).

Факт MVP для старта/reload:
- `/healthz` подтверждает живость HTTP процесса, а не успешность чтения `bot_config.json` или ownership truth.
- `/healthz` не гарантирует readiness `q-sim-service`, readiness NATS или свежесть cached BIOS payload.
- Первый status payload не precomputed заранее: он появляется при первом запросе `/bios/status` или первой успешной publish-итерации.
- `POST /bios/reload` очищает кэш последнего payload; следующий read/publish заново читает текущий `bot_config.json`.
- При `BIOS_PUBLISH_ENABLED=1` freshness `/bios/status` обычно bounded следующей publish-итерацией (`BIOS_PUBLISH_INTERVAL_SEC`), потому что HTTP handler отдаёт cached `_last_payload`.
- При `BIOS_PUBLISH_ENABLED=0` последний HTTP snapshot может оставаться sticky до `POST /bios/reload` или другого пути, который заново вычислит payload.

## 3. Спецификация API
BIOS предоставляет gRPC или простой HTTP API для взаимодействия.

- `get_bios_status() -> bios_status.json`: Возвращает полный отчет о состоянии оборудования.
- `get_component_status(component_id: str) -> component_status_json`: Возвращает статус конкретного компонента.
- `read_port(port_id: str, params: dict) -> PortData`: Читает данные с указанного порта.
- `write_port(port_id: str, command: dict) -> Status`: Отправляет команду на указанный порт.
- `soft_reboot(component_id: str) -> Status`: Инициирует перезагрузку одного компонента.
- `hot_reload_config() -> Status`: Перечитывает `bot_physical_specs.json` для обновления конфигурации портов.

MVP HTTP API (реально сейчас, стабильный v1 contract):
- `GET /healthz`
- `GET /bios/status`
- `GET /bios/component/<id>`
- `POST /bios/reload`

Семантика для операторских/рантайм-доков:
- `GET /healthz` = process liveness only
- `GET /bios/status` = dependency-sensitive cached BIOS snapshot

## 4. Модели Данных

**1. `bios_status.json`:**
```json
{
  "boot_time_utc": "2025-07-21T19:30:00Z",
  "hardware_profile_hash": "sha256:bd1c10a...e4c1",
  "status": "ready", // ready | error
  "components": {
    "motor_left": "ok",
    "lidar_front": "ok",
    "imu_main": "warning: high_drift"
  }
}
```

**2. Контракт Порта (из `bot_physical_specs.json`):**
```json
{
  "port_id": "lidar_front",
  "type": "lidar",
  "protocol": "sim_bus", // sim_bus | i2c | etc.
  "params": {
    "range_m": 100,
    "fov_deg": 180
  }
}
```

## 5. Логирование и Коды Ошибок

**Формат лога (`bios.log`):**
```
[0.01s] BIOS started
[0.02s] Reading hardware profile: sha256:bd1c10a...e4c1
[0.05s] POST: Probing port motor_left... OK
[0.06s] POST: Probing port lidar_front... OK
[0.07s] POST: Probing port imu_main... ERROR (Code: 0x02)
[0.08s] POST complete. STATUS: ERROR
```

**Коды Ошибок (Beep-коды):**
| Код    | Значение                  |
|--------|---------------------------|
| 0x01   | Компонент не найден       |
| 0x02   | Нестабильные показания    |
| 0x03   | Таймаут ответа            |
| 0xF0   | Критический сбой загрузки |

## 6. Анти-паттерны
- **Сложная логика в BIOS:** BIOS не должен содержать никакой бизнес-логики или FSM. Его задача — только инициализация и предоставление доступа.
- **Прямой доступ в обход BIOS:** `Q-Core Agent` никогда не должен пытаться получить доступ к симулятору или железу напрямую.

## 7. Открытые Вопросы
1.  Какой протокол выбрать для API (gRPC или HTTP)? Для MVP предлагается простой HTTP.
2.  Как реализовать `hardware_adapter` для работы с реальным железом в будущем?
