...
- **herdr** — координация/панели. Отправка агенту: **`herdr pane run <pane> "<text>"`** (текст+Enter); чтение `herdr agent read`; статус `herdr agent get`. НЕ `send-text`+`send-keys` (хрупко, не сабмитит).
