...
|---|---|---|---|---|
| Telemetry | позиция/скорость/курс/ориентация/батарея/корпус/радиация/температуры + Thermal nodes + Power/EPS (soc, power in/out, bus V/A) | NATS subject `qiki.telemetry` (`SYSTEM_TELEMETRY`) | JSON dict (TelemetrySnapshot v1; ожидаемые ключи: `schema_version=1`, `source`, `timestamp`, `ts_unix_ms`, `position.{x,y,z}`, `velocity`, `heading` (deg), `attitude.roll_rad`, `attitude.pitch_rad`, `attitude.yaw_rad` (радианы), `battery`, `hull.integrity`, `thermal.nodes[].{id,temp_c}`, `power.soc_pct`, `power.power_in_w`, `power.power_out_w`, `power.bus_v`, `power.bus_a`, `radiation_usvh`, `temp_external_c`, `temp_core_c`) | Таблица со строками‑метриками и значениями `N/A`, `Updated=—`; статус: `waiting for telemetry` |
