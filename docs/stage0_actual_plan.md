...
### 4. Registrar и аудит
- Вынести регистратор в сервис `services/registrar/` как audit/support fan-in/fan-out слой: подписка на выбранные event/radar subjects, запись в `qiki.events.v1.audit` и локальный файл.
...
Каждый шаг закрывается обновлением документации (`CLAUDE_MEMORY.md`, `CURRENT_STATE.md` как snapshot, журналы) и прохождением обязательных проверок (`ruff`, `mypy`, `pytest`).
