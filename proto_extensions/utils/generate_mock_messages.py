...
def gen_sensor_mock():
    reading = sensor_raw_in_pb2.SensorReading(sensor_id="sim_lidar_front", sensor_type=1, scalar_data=3.14)
    write_mock("sensor_reading.mock.json", reading)


def gen_actuator_mock():
    command = actuator_raw_out_pb2.ActuatorCommand(actuator_id="motor_1", command_type="MOVE_FORWARD", confidence=0.8)
