...
reference_guard="$repo_root/scripts/check_reference_truth_boundaries.sh"
if [ -f "$reference_guard" ]; then
  if bash "$reference_guard"; then
    ok_msg "reference truth-boundary guard: PASS"
  else
    warn_msg "reference truth-boundary guard: FAIL (see output above)"
  fi
else
  warn_msg "missing reference truth-boundary guard: $reference_guard"
fi
...
if [ -d "$archive_dir" ]; then
  ...
  ok_msg "archive dir absent: $archive_dir (acceptable when no in-repo archive layer is present)"
fi
