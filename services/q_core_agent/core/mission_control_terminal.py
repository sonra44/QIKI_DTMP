...
class MissionControlTerminal:
    ...
    def _check_system_alerts(self):
        ...
        try:
            ...
            if (
                self.telemetry_data.get("hull_integrity", 100) < 30
                or self.telemetry_data.get("oxygen_level", 21) < 16
                or self.telemetry_data.get("reactor_temp", 0) > 3500
            ):
                self.alert_level = "EMERGENCY"

            # Предупреждения
            elif (
                self.telemetry_data.get("hull_integrity", 100) < 60
                or self.telemetry_data.get("battery_charge", 0)
                / max(self.telemetry_data.get("battery_capacity", 1), 1)
                < 0.2
                or self.telemetry_data.get("main_drive_fuel", 0) < 50
            ):
                self.alert_level = "WARNING"

            # Осторожность
            elif (
                self.telemetry_data.get("hull_integrity", 100) < 80
                or self.telemetry_data.get("co2_level", 0) > 1000
                or self.telemetry_data.get("reactor_temp", 0) > 3000
            ):
            ...
            if prev_alert != self.alert_level:
                self.log(
                    "ALERT SYSTEM",
                    f"🚨 Alert level changed: {prev_alert} → {self.alert_level}",
                )
    ...
    def _create_header(self) -> Window:
        ...
        header_text = FormattedText(
            [
                ...
                ("class:header", " ║\n"),
                ("class:header", "║"),
                ("class:alert", " SYSTEM STATUS: "),
                (self._get_alert_color(), f"{self.alert_level}"),
                ("class:header", "                                           "),
                (
                    "class:mode",
                    f"MODE: {'AUTO' if self.autopilot_enabled else 'MANUAL'}",
                ),
                ("class:header", " ║\n"),
                (
                    "class:header",
    ...
    def _create_telemetry_panel(self) -> Window:
        ...
        reactor_pct = (
            self.telemetry_data.get("reactor_output", 0)
            / max(self.telemetry_data.get("reactor_max", 1), 1)
        ) * 100
        battery_pct = (
            self.telemetry_data.get("battery_charge", 0)
            / max(self.telemetry_data.get("battery_capacity", 1), 1)
        ) * 100
        ...
        telemetry_text = FormattedText(
            [
                ...
                (
                    "ansigreen"
                    if self.telemetry_data.get("oxygen_level", 0) > 18
                    else "ansired",
                ...
                (
                    "ansigreen"
                    if self.telemetry_data.get("co2_level", 0) < 1000
                    else "ansiyellow",
                ...
                (
                    "ansigreen"
                    if self.telemetry_data.get("qiki_status") == "active"
                    else "ansired",
    ...
    def _create_propulsion_panel(self) -> Window:
        ...
        propulsion_text = FormattedText(
            [
                ...
                (
                    "ansigreen"
                    if drive_status in ["ready", "idle", "active"]
                    else "ansired",
                    f"{drive_status:>8}",
                ),
                ("class:panel", "                │\n"),
                ...
                (
                    "ansiblue"
                    if "FLIGHT" in self.telemetry_data.get("ship_state", "")
                    else "ansigreen",
    ...
    def _create_layout(self) -> Layout:
        ...
        command_input = Frame(self._create_command_input(), title="COMMAND INTERFACE")
        ...
        try:
            ...
            if cmd == "thrust":
                if len(cmd_parts) >= 2:
                    ...
                    if success:
                        self.log(
                            "PROPULSION", f"🚀 Main drive thrust set to {thrust_pct}%"
                        )
                    else:
                        self.log(
                            "PROPULSION", f"❌ Failed to set thrust to {thrust_pct}%"
                        )
                else:
            ...
            # RCS команды
                ...
                if len(cmd_parts) >= 3:
                    ...
                    if direction in axis_map:
                        ...
                        if success:
                            self.log(
                                "RCS", f"🎯 {direction} thruster fired at {thrust_pct}%"
                            )
                        else:
                        ...
                        self.log(
                            "COMMAND",
                            "❌ Invalid RCS direction. Use: forward, backward, port, starboard",
                        )
                else:
            ...
            # Управление питанием
                ...
                if len(cmd_parts) >= 2:
                    if cmd_parts[1] == "status":
                        ...
                        self.log(
                            "POWER",
                            f"⚡ Reactor: {power_status.reactor_output_mw:.1f}MW",
                        )
                        self.log(
                            "POWER",
                            f"⚡ Battery: {power_status.battery_charge_mwh:.1f}MWh",
                        )
                    else:
            ...
            # Управление сенсорами
                ...
                if len(cmd_parts) >= 3:
                    ...
                    if action == "activate":
                        ...
                        self.log(
                            "COMMAND",
                            "❌ Usage: sensor <activate|deactivate> <sensor_id>",
                        )
                else:
                    self.log(
                        "COMMAND", "❌ Usage: sensor <activate|deactivate> <sensor_id>"
                    )
            ...
            # Статус систем
                ...
                self.log(
                    "STATUS", f"🛰️ Ship State: {summary.get('current_state', 'UNKNOWN')}"
                )
                self.log(
                    "STATUS", f"🛰️ Systems: {summary.get('overall_status', 'UNKNOWN')}"
                )
                self.log(
                    "STATUS",
                    f"🛰️ Navigation: {'OK' if summary.get('navigation_capable') else 'LIMITED'}",
                )
            ...
            # Справка
                ...
                self.log(
                    "HELP", "💡 Hotkeys: F1=Autopilot, F2=Emergency, F3=Diagnostics"
                )
