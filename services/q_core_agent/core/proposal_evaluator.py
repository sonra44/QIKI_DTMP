...
class ProposalEvaluator(IProposalEvaluator):
    ...
    def evaluate_proposals(self, proposals: List[Proposal]) -> List[Proposal]:
        ...
        if best_proposal:
            ...
            logger.info(f"Accepted proposal: {best_proposal.proposal_id.value} (Source: {best_proposal.source_module_id}, Type: {best_proposal.type}, Priority: {best_proposal.priority}, Confidence: {best_proposal.confidence})")
        else:
