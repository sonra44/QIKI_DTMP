...
from ship_actuators import ShipActuatorController
...
class QIKIMissionControlFixed:
    ...
    def __init__(self):
        ...
        self.update_thread = threading.Thread(
            target=self._update_live_params, daemon=True
        )
    ...
    def _update_live_params(self):
        ...
        while self.running:
            try:
                ...
                self.live_params["reactor_output"] = max(
                    30.0, min(45.0, self.live_params["reactor_output"])
                )

                # Температура реактора
                self.live_params["reactor_temp"] += random.uniform(-10, 10)
                self.live_params["reactor_temp"] = max(
                    2750, min(2850, self.live_params["reactor_temp"])
                )
                ...
                if self.live_params["thruster_power"] > 0:
                    ...
                    self.live_params["fuel_remaining"] = max(
                        0, self.live_params["fuel_remaining"]
                    )
                ...
                self.live_params["battery_charge"] = max(
                    0, min(12.0, self.live_params["battery_charge"])
                )
                self.live_params["battery_percent"] = (
                    self.live_params["battery_charge"] / 12.0
                ) * 100
                ...
                if self.live_params["mission_progress"] < 100:
                    ...
                    # ETA уменьшается
                    total_minutes = (
                        self.live_params["eta_hours"] * 60
                        + self.live_params["eta_minutes"]
                    )
    ...
    def _process_command(self, command_text: str):
        ...
        try:
            if cmd == "thrust":
                ...
                self.log(
                    "STATUS", f"⚡ Power: {self.live_params['reactor_output']:.1f}MW"
                )
                self.log(
                    "STATUS", f"🎯 Mission: {self.live_params['mission_progress']:.1f}%"
                )

            elif cmd in ["help", "?"]:
                self.log(
                    "HELP",
                    "📚 Commands: thrust <0-100>, autopilot, lang, emergency, status",
                )
