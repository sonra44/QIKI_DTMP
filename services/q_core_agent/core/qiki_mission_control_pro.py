...
class QIKIMissionControlPro:
    ...
    def __init__(self):
        ...
        self.log("СИСТЕМА / SYSTEM", "✅ QIKI Mission Control Pro инициализирован / initialized")
    ...
    def _update_live_telemetry(self, dt: float):
        ...
        try:
            ...
            self.live_telemetry = {
                ...
                'anomaly_distance': 32.17 - (self.simulation_time * 0.01),  # Приближаемся
    ...
    def _create_navigation_window(self) -> Window:
        ...
        main_thrust = 20 if self.actuator_controller.current_mode == PropulsionMode.CRUISE else 0
        ...
        nav_text = FormattedText([
            ...
            ("class:panel", f"│ │                               │/\n"),
            ("class:panel", f"│ ╰─ Сектор: Дельта-5      ──────────────────────────>\n"),
            ("class:panel", f"│                          O КИКИ      X\n"),
            ("class:panel", f"│\n"),
            ("class:panel", f"│ {velocity_title}\n"),
            ("class:panel", f"│ ├─ Абсолютная: {vel_abs:4.0f} м/с\n"),
            ("class:panel", f"│ ├─ Относительная: {vel_rel:+3.0f} м/с (к цели)\n"),
            ("class:panel", f"│ ╰─ Дрейф: 0.03%/ч (стабильно)\n"),
            ("class:panel", f"│\n"),
            ("class:panel", f"│ {thrusters_title}\n"),
            ("class:panel", f"│ ├─ Главные:    [{self._format_bar(main_thrust)}] {main_thrust:2.0f}%\n"),
    ...
    def _create_telemetry_window(self) -> Window:
        ...
        tel_text = FormattedText([
            ...
            ("ansigreen" if qiki_status == 'active' else "ansired", f"{qiki_status:>8}"),
            ("class:panel", "             │\n"),
    ...
    def _create_mission_window(self) -> Window:
        ...
        mission_text = FormattedText([
            ...
            ("class:panel", f"│ ├─ Цель: {self.mission_data['objective'][:25]}...\n"),
            ("class:panel", f"│ ├─ Прогресс: [{self._format_bar(progress)}] {progress:4.1f}%\n"),
