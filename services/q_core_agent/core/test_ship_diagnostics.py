...
def test_ship_diagnostics():
    ...
    try:
        ...
        print(f"   Reactor: {power.reactor_output_mw:.1f}/{power.reactor_max_output_mw:.1f} MW ({power.reactor_output_mw/power.reactor_max_output_mw*100:.1f}%)")
        print(f"   Fuel: {power.reactor_fuel_hours:.1f} hours remaining")
        print(f"   Temperature: {power.reactor_temperature_k:.0f}K")
        print(f"   Battery: {power.battery_charge_mwh:.1f}/{power.battery_capacity_mwh:.1f} MWh ({power.battery_charge_mwh/power.battery_capacity_mwh*100:.1f}%)")
        ...
        for thruster_id, thruster_data in propulsion.rcs_status.items():
            print(f"     {thruster_id}: {thruster_data['status']}, {thruster_data['fuel_kg']}kg, {thruster_data['thrust_n']}N")
        print()
        ...
        for sensor_id, sensor_data in sensors.sensor_data.items():
            ...
            print(f"     {status_indicator} {sensor_id}: {sensor_data['type']}, {sensor_data['status']}, {sensor_data['range_km']}km range")
        print()
        
        print("5. LIFE SUPPORT:")
        life_support = ship.get_life_support_status()
        atm = life_support.atmosphere
        print(f"   Atmosphere: {atm['oxygen_percent']:.1f}% O2, {atm['nitrogen_percent']:.1f}% N2, {atm['co2_ppm']} ppm CO2")
        print(f"   Pressure: {atm['pressure_kpa']:.1f} kPa, Temperature: {atm['temperature_k']:.0f}K, Humidity: {atm['humidity_percent']:.1f}%")
        water = life_support.water_recycling
        print(f"   Water: {water['clean_water_liters']} L clean, {water['waste_water_liters']} L waste, {water['recycling_efficiency']:.1%} efficiency")
        air = life_support.air_recycling
        print(f"   Air Recycling: {air['active_scrubbers']}/{air['co2_scrubbers']} CO2 scrubbers, {air['oxygen_generators']} O2 generators")
        print()
