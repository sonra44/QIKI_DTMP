...
class TestAsyncStateStoreBasics:
    ...
    async def test_version_auto_increment(self, empty_store):
        ...
        snap2 = FsmSnapshotDTO(version=1, state=FsmState.IDLE, reason="SECOND")  # та же версия
    ...
    async def test_version_enforcement(self, empty_store):
        ...
        snap2 = FsmSnapshotDTO(version=3, state=FsmState.ACTIVE, reason="SECOND")  # старая версия
...
class TestAsyncStateStorePubSub:
    ...
    async def test_multiple_subscribers(self, empty_store):
        ...
        test_snapshot = FsmSnapshotDTO(version=1, state=FsmState.ACTIVE, reason="MULTI_TEST")
        await empty_store.set(test_snapshot)
    ...
    async def test_unsubscribe(self, empty_store):
        ...
        test_snapshot = FsmSnapshotDTO(version=1, state=FsmState.IDLE, reason="UNSUB_TEST")
        await empty_store.set(test_snapshot)
    ...
    async def test_queue_overflow_handling(self, empty_store):
        ...
        for i in range(70):  # больше чем maxsize
            snapshot = FsmSnapshotDTO(version=i+1, state=FsmState.IDLE, reason=f"OVERFLOW_{i}")
    ...
    async def test_dead_subscriber_cleanup(self, empty_store):
        ...
        test_snapshot = FsmSnapshotDTO(version=1, state=FsmState.IDLE, reason="CLEANUP_TEST")
        await empty_store.set(test_snapshot)
...
class TestAsyncStateStoreConcurrency:
    ...
    async def test_mixed_operations_stress(self, empty_store):
        ...
        async def writer():
            for i in range(10):
                snapshot = FsmSnapshotDTO(version=i*100, state=FsmState.ACTIVE, reason="STRESS")
                await empty_store.set(snapshot)
        ...
        await asyncio.gather(
            reader(), reader(), reader(),  # 3 читателя
            writer(), writer(),            # 2 писателя  
            subscriber(), subscriber()     # 2 подписчика
        )
...
class TestAsyncStateStoreErrorHandling:
    ...
    async def test_concurrent_access_safety(self, empty_store):
        ...
        async def risky_operation(i):
            try:
                snapshot = FsmSnapshotDTO(version=i, state=FsmState.IDLE, reason=f"RISKY_{i}")
                await empty_store.set(snapshot)
                result = await empty_store.get()
