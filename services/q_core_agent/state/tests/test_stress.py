...
class TestHighVolumeOperations:
    ...
    async def test_rapid_state_transitions(self, stress_store):
        ...
        with PerformanceMonitor("Rapid State Transitions") as monitor:
            ...
            states = [FsmState.IDLE, FsmState.ACTIVE, FsmState.ERROR_STATE, FsmState.IDLE]
            ...
            try:
                for i in range(50):  # 50 операций на писателя
                    snapshot = FsmSnapshotDTO(
                        ...
                        context_data={f"writer_id": str(writer_id), f"op": str(i)}
                    )
    ...
    async def test_mixed_operations_chaos(self, stress_store):
        ...
        with PerformanceMonitor("Mixed Operations Chaos") as monitor:
            ...
            await asyncio.gather(
                reader_task(), reader_task(), reader_task(),  # 3 читателя
                writer_task(), writer_task(),                 # 2 писателя
                subscriber_task(), subscriber_task()          # 2 подписчика
            )
            
            monitor.operations = results['reads'] + results['writes'] + results['subscribes']
    ...
    async def test_subscriber_memory_cleanup(self, stress_store):
        ...
        with PerformanceMonitor("Subscriber Memory Cleanup") as monitor:
            ...
            for wave in range(10):
                ...
                metrics = await stress_store.get_metrics()
                assert metrics['active_subscribers'] == 0, f"Wave {wave}: subscribers not cleaned up"
...
class TestLongRunningStability:
    ...
    async def test_long_running_operations(self, stress_store):
        ...
        print(f"Long running test: {operation_count} operations in {STRESS_TEST_DURATION}s")
...
class TestErrorHandlingStress:
    ...
    async def test_resource_exhaustion_recovery(self, stress_store):
        ...
        try:
            snapshot = FsmSnapshotDTO(version=1, state=FsmState.IDLE, reason="RECOVERY_TEST")
            await stress_store.set(snapshot)
...
class TestPerformanceBenchmarks:
    ...
    async def test_throughput_benchmark(self, stress_store):
        ...
        for i in range(operations):
            snapshot = FsmSnapshotDTO(version=i, state=FsmState.IDLE, reason="BENCHMARK")
            await stress_store.set(snapshot)
        ...
        assert write_ops_per_sec > 1000, f"Write performance too low: {write_ops_per_sec:.1f}"
        assert read_ops_per_sec > 5000, f"Read performance too low: {read_ops_per_sec:.1f}"
    ...
    async def test_latency_benchmark(self, stress_store):
        ...
        for i in range(operations):
            ...
            snapshot = FsmSnapshotDTO(version=i, state=FsmState.ACTIVE, reason="LATENCY")
