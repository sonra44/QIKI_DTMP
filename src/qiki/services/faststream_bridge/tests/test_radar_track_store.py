...
def test_track_store_preserves_track_id_across_sr_label_mutation():
    store = RadarTrackStore()
    start = datetime.now(UTC)
    ...
    initial_track = store.process_frame(first)[0]

    mutated = _frame_with_detection(
        range_m=42.4,
        bearing_deg=12.1,
        elev_deg=2.0,
        vr_mps=-1.0,
        ...
        transponder_id="HOSTILE-777",
        id_present=True,
    )
    updated_track = store.process_frame(mutated)[0]

    assert updated_track.track_id == initial_track.track_id
    assert initial_track.transponder_id == "ALLY-001"
    assert updated_track.transponder_id == "HOSTILE-777"
    assert updated_track.transponder_mode == TransponderModeEnum.SPOOF


def test_track_store_spawns_new_track_when_mutated_contact_breaks_association():
    store = RadarTrackStore()
    start = datetime.now(UTC)
    ...
    mutated_far = _frame_with_detection(
        range_m=120.0,
        bearing_deg=40.0,
        elev_deg=8.0,
        vr_mps=9.0,
    ...
    tracks = store.process_frame(mutated_far)
    track_ids = {track.track_id for track in tracks}

    assert initial_track.track_id in track_ids
    assert len(track_ids) == 2
    assert any(track.transponder_id == "HOSTILE-777" for track in tracks if track.track_id != initial_track.track_id)
