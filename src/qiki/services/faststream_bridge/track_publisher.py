...
class RadarTrackPublisher:
    ...
    @staticmethod
    def build_event_id(track: RadarTrackModel) -> str:
        """
        Return a unique event id for each publish attempt while preserving
        stable contact continuity in the payload's `track_id`.

        JetStream deduplicates by `Nats-Msg-Id`, so reusing bare `track_id`
        suppresses real same-track mutations like ON -> SPOOF label changes.
        """
        event_ts = track.ts_ingest or track.ts_event or track.timestamp
        if event_ts.tzinfo is None:
            event_ts = event_ts.replace(tzinfo=UTC)
        stamp = event_ts.isoformat(timespec="microseconds").replace("+00:00", "Z")
        return f"{track.track_id}:{stamp}"
    ...
    def build_headers(track: RadarTrackModel, *, extra_headers: dict[str, str] | None = None) -> dict[str, str]:
        event_id = RadarTrackPublisher.build_event_id(track)
