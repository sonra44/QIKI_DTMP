...
class MetricsClient:
    ...
    async def _collect_system_metrics(self, timestamp: datetime):
        ...
        try:
            ...
            self.add_metric(
                "system.cpu.usage_percent", cpu_percent, timestamp, unit="%", description="CPU usage percentage"
            )
            ...
            try:
                ...
                self.add_metric(
                    "system.disk.usage_percent", disk_percent, timestamp, unit="%", description="Disk usage percentage"
                )
        ...
        try:
            if getattr(client, "is_connected", False):
                ...
                if hasattr(client, "messages_processed"):
                    self.add_metric(
                        "nats.messages.processed_total",
                        float(getattr(client, "messages_processed", 0)),
                        timestamp,
                        description="Total messages processed",
                    )
            else:
                self.add_metric("nats.connection.status", 0.0, timestamp, description="NATS connection status")
    ...
    async def _collect_grpc_metrics(self, name: str, client: Any, timestamp: datetime):
        ...
        try:
            ...
            self.add_metric(
                f"grpc.{name}.connection.status", connected, timestamp, description=f"{name} gRPC connection status"
            )
            ...
            if hasattr(client, "health_check") and connected:
                ...
                try:
                    ...
                    self.add_metric(
                        f"grpc.{name}.health_check.latency_ms",
                        latency_ms,
                        timestamp,
                        unit="ms",
                        description=f"{name} health check latency",
                    )
                except Exception:
                    # Health check failed
                    self.add_metric(
                        ...
                        timestamp,
                        unit="ms",
                        description=f"{name} health check latency (failed)",
                    )
    ...
    async def _collect_app_metrics(self, timestamp: datetime):
        ...
        try:
            ...
            self.add_metric("app.uptime_seconds", uptime_seconds, timestamp, unit="s", description="Application uptime")

            # Collection frequency
            collection_interval = time.time() - self.last_collection
            self.add_metric(
                "app.metrics.collection_interval_seconds",
                collection_interval,
                timestamp,
                unit="s",
                description="Time between metric collections",
            )

            # Number of tracked metrics
            metrics_count = len(self.metrics)
            self.add_metric(
                "app.metrics.tracked_count", float(metrics_count), timestamp, description="Number of tracked metrics"
            )
    ...
    def add_metric(
        ...
        if name not in self.metrics:
            self.metrics[name] = MetricSeries(
                name=name, points=deque(maxlen=self.max_points), unit=unit, description=description
            )
    ...
    def _export_json(self) -> str:
        ...
        export_data: Dict[str, Any] = {"timestamp": datetime.now().isoformat(), "metrics": {}}

        for name, series in self.metrics.items():
            points_data = []
            for point in list(series.points)[-100:]:  # Last 100 points
                points_data.append(
                    {"timestamp": point.timestamp.isoformat(), "value": point.value, "labels": point.labels}
                )
