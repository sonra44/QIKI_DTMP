...
def create_radar_table():
    ...
    for track in tracks:
        table.add_row(track["ID"], track["Range"], track["Bearing"], track["Speed"], track["Type"], track["Status"])
...
def create_layout():
    ...
    # Header
    header_text = Align.center(Text("QIKI OPERATOR CONSOLE v1.0", style="bold white on blue"), vertical="middle")
    ...
    layout["left"].split_column(Layout(name="telemetry"), Layout(name="radar"))

    # Right side - status and commands
    layout["right"].split_column(Layout(name="status"), Layout(name="commands", size=10))

    # Footer
    footer_text = Align.center(
        Text(f"Connected to QIKI Digital Twin | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", style="dim"),
        vertical="middle",
    )
    ...
    try:
        with Live(create_layout(), refresh_per_second=2, screen=True) as live:
            while True:
                ...
                footer_text = Align.center(
                    Text(
                        f"Connected to QIKI Digital Twin | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", style="dim"
                    ),
                    vertical="middle",
                )
                layout["footer"].update(footer_text)
