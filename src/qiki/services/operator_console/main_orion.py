...
ORION_CRITICAL_EXCEPTION_COUNT = 0


def _log_critical_exception(*, component: str, action: str, exc: BaseException, **context: Any) -> None:
    """Unified fail-loud handler for broad-exception branches in critical paths."""
    global ORION_CRITICAL_EXCEPTION_COUNT
    ORION_CRITICAL_EXCEPTION_COUNT += 1
    extra_ctx = " ".join(f"{k}={v!r}" for k, v in context.items() if v is not None)
    msg = f"orion_critical_exception component={component} action={action}"
    if extra_ctx:
        msg = f"{msg} {extra_ctx}"
    logger.exception("%s err=%s", msg, exc)


def _safe_json_loads(raw: bytes | str, *, component: str, action: str, subject: str = "") -> Any | None:
    """Decode JSON with critical-path logging and safe payload diagnostics."""
    try:
        return json.loads(raw)
    except Exception as exc:
        payload_bytes = raw if isinstance(raw, bytes) else raw.encode("utf-8", errors="replace")
        _log_critical_exception(
            ...
            payload_head_hex=payload_bytes[:64].hex(),
        )
        return None
    ...
    def on_unmount(self) -> None:
        if self._task is not None:
            try:
                ...
                _log_critical_exception(
                    component="ui",
                    action="restore_command_dock_after_secret_entry",
                    exc=exc,
                )
        ...
        try:
            ...
            if buttons is not None:
                try:
                    ...
                    _log_critical_exception(
                        component="ui",
                        action="enter_secret_entry_mode",
                        exc=exc,
                    )
...
class OrionApp(App):
        ...
        def rebuild() -> None:
            ...
            if cursor_key is not None and cursor_key in desired_keys:
                try:
                    ...
                    _log_critical_exception(
                        component="ui",
                        action="cancel_secret_entry_mode",
                        exc=exc,
                    )
    ...
    async def _init_nats(self) -> None:
        ...
        try:
            ...
            _log_critical_exception(component="nats", action="connect", exc=e, nats_url=getattr(self.nats_client, "url", None))
        ...
        try:
            ...
            _log_critical_exception(component="nats", action="subscribe_system_telemetry", exc=e, subject=SYSTEM_TELEMETRY)
        ...
        try:
            ...
            _log_critical_exception(component="nats", action="subscribe_tracks", exc=e, subject=RADAR_TRACKS)
        ...
        try:
            ...
            _log_critical_exception(component="nats", action="subscribe_events", exc=e, subject=EVENTS_V1_WILDCARD)
        ...
        try:
            ...
            _log_critical_exception(component="nats", action="subscribe_control_responses", exc=e, subject=RESPONSES_CONTROL)
            self._log_msg(
        ...
        try:
            ...
            _log_critical_exception(component="nats", action="subscribe_qiki_responses", exc=e)
    ...
    async def _hydrate_qiki_mode_from_jetstream(self) -> None:
        ...
        try:
            ...
            _log_critical_exception(
                component="nats",
                action="hydrate_qiki_mode_from_jetstream",
                exc=exc,
        ...
        if isinstance(mode, str) and mode.strip():
            ...
            try:
                ...
                _log_critical_exception(component="ui", action="header_update_mode_after_hydrate", exc=exc)
                return
    ...
    async def handle_event_data(self, data: dict) -> None:
        ...
        if etype == "bios" and not self._bios_loaded_announced:
            ...
            if isinstance(payload, dict):
                ...
                if isinstance(post_results, list):
                    device_count = len(post_results)
            msg = f"{I18N.bidi('BIOS loaded', 'BIOS загрузился')}: {status_label}"
            if device_count is not None:
                msg = f"{msg} ({I18N.bidi('devices', 'устройств')}: {device_count})"
        ...
        if matched_incidents and self.nats_client:
            for inc in matched_incidents:
                ...
                try:
                    ...
                    _log_critical_exception(
                        component="audit",
                        action="publish_incident_open_audit",
                        exc=exc,
                        kind="incident_open",
                    )
                    self._console_log(
        ...
        if self._events_live:
            ...
            try:
                ...
                _log_critical_exception(component="ui", action="events_paused_refresh_sidebar", exc=exc)
            try:
                self.query_one("#orion-keybar", OrionKeybar).refresh()
            except Exception as exc:
                _log_critical_exception(component="ui", action="events_paused_refresh_keybar", exc=exc)
    ...
    def action_toggle_events_live(self) -> None:
        ...
        try:
            ...
            _log_critical_exception(component="ui", action="toggle_events_live_refresh_sidebar", exc=exc)
        try:
            self.query_one("#orion-keybar", OrionKeybar).refresh()
        except Exception as exc:
            _log_critical_exception(component="ui", action="toggle_events_live_refresh_keybar", exc=exc)
    ...
    def _confirm_power_toggle(self, *, row_key: str, current: object) -> None:
        ...
        def after(decision: bool | None) -> None:
            ...
            try:
                ...
                _log_critical_exception(
                    component="control",
                    action="schedule_power_toggle_command",
                    exc=exc,
                    command=cmd,
                    row_key=key,
                )
    ...
    def action_mark_events_read(self) -> None:
        ...
        try:
            ...
            _log_critical_exception(component="ui", action="mark_events_read_refresh_sidebar", exc=exc)
        try:
            self.query_one("#orion-keybar", OrionKeybar).refresh()
        except Exception as exc:
            _log_critical_exception(component="ui", action="mark_events_read_refresh_keybar", exc=exc)
        if self.active_screen == "events":
            self._refresh_inspector()
    ...
    async def handle_qiki_response(self, data: dict) -> None:
        ...
        try:
            ...
            _log_critical_exception(
                component="qiki",
                action="decode_response",
                exc=exc,
                kind=kind,
                request_id=request,
                payload_size=len(json.dumps(payload, ensure_ascii=False)) if isinstance(payload, dict) else None,
            )
        ...
        if resp.mode:
            ...
            try:
                ...
                _log_critical_exception(
                    component="ui",
                    action="update_mode_from_qiki_response",
                    exc=exc,
                    request_id=str(resp.request_id),
                )
    ...
    def action_show_screen(self, screen: str) -> None:
        ...
        try:
            ...
            _log_critical_exception(component="ui", action="show_screen_set_sidebar_active", exc=exc, screen=screen)
        try:
            # Keybar is a pure renderer; it won't necessarily re-render on app state changes
            # unless explicitly refreshed.
            self.query_one("#orion-keybar", OrionKeybar).refresh()
        except Exception as exc:
            _log_critical_exception(component="ui", action="show_screen_refresh_keybar", exc=exc, screen=screen)
        for sid in (
            "system",
            "radar",
            "events",
    ...
    async def _run_command(self, raw: str) -> None:
            ...
            if action in {"start", "on"}:
                ...
                def _on_done(t: asyncio.Task) -> None:
                    ...
                    try:
                        ...
                        _log_critical_exception(
                            component="record",
                            action="record_task_done_callback",
                            exc=exc,
        ...
        if low == "replay" or low.startswith("replay "):
                ...
                if "=" in tok:
                    ...
                    if k == "speed":
                        try:
                            ...
                            self._console_log(
                                f"{I18N.bidi('Invalid replay speed', 'Некорректная скорость replay')}: {v or I18N.NA}",
                                level="warn",
                            )
                        continue
            ...
            def _on_done(t: asyncio.Task) -> None:
                ...
                try:
                    ...
                    _log_critical_exception(
                        component="replay",
                        action="replay_task_done_callback",
                        exc=exc,
                        input_path=self._replay_input_path,
                    )
            ...
            if self._incident_store is not None:
                for inc in list(self._incident_store.list_incidents()):
                    try:
                        ...
                        _log_critical_exception(
                            component="incidents",
                            action="snapshot_acked_incident",
                            exc=exc,
                        )
                        continue
            cleared = self._clear_acked_incidents()
        ...
        if low.startswith(("radar.view", "радар.вид")):
            ...
            try:
                ...
                _log_critical_exception(component="radar", action="render_after_view_change", exc=exc, view=view)
            return
        ...
        if low.startswith(("radar.iso rotate", "радар.изо повернуть")):
            ...
            try:
                ...
                _log_critical_exception(
                    component="radar",
                    action="render_after_iso_rotate",
                    exc=exc,
                    yaw=self._radar_iso_yaw_deg,
                    pitch=self._radar_iso_pitch_deg,
                )
            return
        ...
        if low in {"radar.iso reset", "радар.изо сброс"}:
            ...
            try:
                ...
                _log_critical_exception(component="radar", action="render_after_iso_reset", exc=exc)
            return
        ...
        if low.startswith(("radar.zoom", "радар.зум", "радар.масштаб")):
            ...
            try:
                ...
                _log_critical_exception(
                    component="radar",
                    action="render_after_zoom_change",
                    exc=exc,
                    zoom=round(float(self._radar_zoom), 3),
                )
            return
        ...
        if low in {"radar.pan reset", "радар.пан сброс", "радар.сдвиг сброс"}:
            ...
            try:
                ...
                _log_critical_exception(component="radar", action="render_after_pan_reset", exc=exc)
            return
    ...
    async def _publish_qiki_proposal_decision(self, *, proposal_id: str, decision: str) -> None:
        ...
        try:
            ...
            _log_critical_exception(
                component="qiki",
                action="publish_proposal_decision",
                exc=e,
                subject=QIKI_INTENTS,
                request_id=str(req.request_id),
                proposal_id=pid,
    ...
    def _update_command_placeholder(self) -> None:
        try:
            ...
            _log_critical_exception(component="ui", action="update_command_placeholder_query_input", exc=exc)
            return
    ...
    def on_input_changed(self, event: Input.Changed) -> None:
        ...
        try:
            ...
            _log_critical_exception(
                component="ui",
                action="input_trim_write",
                exc=exc,
                max_chars=self._command_max_chars,
            )
    ...
    async def _publish_audit_event(self, payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            _log_critical_exception(
                component="audit",
                action="publish_operator_action",
                exc=exc,
