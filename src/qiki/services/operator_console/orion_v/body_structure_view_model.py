    ...
    *,
    ...
    if decision is None:
        return BodyStructureConsoleViewModel(
            ...
            read_only=True,
            operator_summary="Улик установки пока нет. B — запустить самопроверку установки.",
            evidence_card=None,
...
def format_body_structure_system_summary(vm: BodyStructureConsoleViewModel | None = None) -> str:
    ...
    if vm.last_decision == "waiting":
        return "\n".join(
            [
                "Корпус / Модули",
                f"Статус            {vm.seed_status}",
                f"Режим             {vm.mode}",
                f"Транспорт         {vm.transport}",
                f"Грани             {vm.faces_range} ({vm.faces_total})",
                f"Модулей           {vm.attached_modules_count}",
                f"{BODY_STRUCTURE_TEST_MOUNT}               {vm.after_mount_state}",
                *face_map_lines,
                *selected_lines,
                f"Действие          {vm.last_action}",
                f"Решение           {vm.last_decision}",
                "Улики             пока нет",
                f"Дальше            {vm.operator_hint}",
                "Модули",
                *module_lines,
            ]
        )
    return "\n".join(
        [
            "Корпус / Модули",
            f"Статус посева     {vm.seed_status}",
            f"Статус            {vm.seed_status}",
            f"Режим             {vm.mode}",
            f"Транспорт         {vm.transport}",
            f"Грани             {vm.faces_range} ({vm.faces_total})",
            ...
            *selected_lines,
            ...
            f"  {BODY_STRUCTURE_TEST_MOUNT}             {vm.after_mount_state}",
            "Модуль",
            f"Модуль            {vm.module_id or 'none'}",
            f"Гнездо            {vm.mount_point}",
            f"Готов к работе    {str(vm.runtime_ready).lower()}",
            f"Способность       {vm.capability_status}",
            f"Источник          {vm.source}",
            f"Доверие           {vm.trust_status}",
            f"Улики             {vm.evidence_card_id or 'none'}",
