...
def run_tests(args):
    ...
    if args.coverage:
        cmd.extend(
            ["--cov=.", "--cov-report=html:coverage_html", "--cov-report=term-missing", "--cov-report=xml:coverage.xml"]
        )
        if args.cov_fail:
...
def run_linting(args):
    ...
    if args.ruff or args.all_lint:
        ...
        success &= run_command(
            [
                ...
                ".",
                "--config",
                "pyproject.toml" if Path("pyproject.toml").exists() else "setup.cfg",
            ],
            "Ruff linting",
        )
    ...
    if args.pylint or args.all_lint:
        # Run pylint
        success &= run_command(
            ["python", "-m", "pylint", ".", "--rcfile=.pylintrc" if Path(".pylintrc").exists() else "--disable=all"],
            "Pylint analysis",
        )
...
def run_security_checks():
    ...
    try:
        success &= run_command(["python", "-m", "pip", "audit"], "Security audit")
