...
class MetricCard(Static):
    ...
    def render(self) -> RenderableType:
        ...
        border_style = {"normal": "green", "warning": "yellow", "critical": "red"}.get(self.metric_status, "white")
        ...
        return Panel(panel_content, title=f"📊 {self.metric_name}", border_style=border_style, height=10)
...
class SystemOverviewCard(Static):
    ...
    def render(self) -> RenderableType:
        ...
        if not self.metrics:
            return Panel("No system metrics available", title="🖥️ System Overview", border_style="dim")
        ...
        table.add_row("NATS", "Connected" if nats_status else "Disconnected", "🟢 OK" if nats_status else "🔴 Down")
        table.add_row(
            "Q-Sim gRPC", "Connected" if grpc_sim_status else "Disconnected", "🟢 OK" if grpc_sim_status else "🔴 Down"
        )
        table.add_row(
            "Q-Agent gRPC",
            "Connected" if grpc_agent_status else "Disconnected",
            "🟢 OK" if grpc_agent_status else "🔴 Down",
        )

        return Panel(table, title="🖥️ System Overview", border_style="bright_blue")
...
class MetricsPanel(ScrollableContainer):
    ...
    async def update_metrics(self):
        ...
        try:
            ...
            grpc_latencies = [v for k, v in latest.items() if "grpc." in k and "latency_ms" in k and v > 0]
            if grpc_latencies:
