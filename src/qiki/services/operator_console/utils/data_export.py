...
class DataExporter:
    ...
    def export_to_csv(
        self, data: List[Dict[str, Any]], filename: Optional[str] = None, headers: Optional[List[str]] = None
    ) -> str:
        """
    ...
    def export_telemetry(self, telemetry_buffer: Dict[str, Any], format: str = "json") -> str:
        """
        ...
        if format == "json":
            json_payload = {"export_timestamp": datetime.now().isoformat(), "telemetry": telemetry_buffer}
            ...
            for key, value in telemetry_buffer.items():
                rows.append({"timestamp": datetime.now().isoformat(), "metric": key, "value": str(value)})
    ...
    def create_report(self, session_data: Dict[str, Any]) -> str:
        ...
        if commands:
            report.append("\nRECENT COMMANDS:")
            for cmd in commands:
                report.append(f"  [{cmd.get('timestamp', 'N/A')}] {cmd.get('command', '')}")
...
if __name__ == "__main__":
    ...
    telemetry = {"position_x": 10.5, "position_y": 20.3, "velocity": 5.2, "battery": 85}
    ...
    session_data = {
        ...
        "uptime_seconds": 300,
        "stats": {"frames_received": 1234, "events_received": 56, "commands_sent": 12},
