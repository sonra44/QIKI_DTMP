...
class BiosHttpHandler(BaseHTTPRequestHandler):
    """Minimal stdlib HTTP contract for the support-tier BIOS status layer."""
    ...
    def do_POST(self) -> None:  # noqa: N802
        if self.path == "/bios/reload":
            # v1 reload only invalidates cached derived payload and lets the next
            # HTTP read / publish cycle recompute from the configured sources.
