...
class BiosService:
    """Support-tier derived BIOS/status projection over bot config plus q-sim health."""
    ...
    def _compute_payload(self) -> dict[str, Any]:
        # q-bios-service derives support status from the configured hardware
        # profile plus q-sim health; it is not an owner of runtime truth or intents.
...
def main() -> None:
    ...
    logger.info(
        "q-bios-service role: support-tier derived BIOS/status layer; not runtime truth owner and not intents owner"
    )
