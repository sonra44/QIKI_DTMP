...
class PluginManager:
        ...
        *,
        ...
        try:
            ...
            config_hash = self._config_hash(
                {"profile": selected_profile, "selection": selections, "source": "fallback"}
            )
            instances = self._instantiate_selected(selections, config_hash=config_hash)
            self._append_lifecycle_event(
            ...
            return LoadedPlugins(
                instances=instances, profile=selected_profile, source="fallback", warning_reason=warning
            )
    ...
    def _load_plugins_doc(self, path: str) -> tuple[dict[str, Any], str]:
        if path:
            ...
            text = (
                resources.files(DEFAULT_PLUGINS_RESOURCE_PACKAGE)
                .joinpath(DEFAULT_PLUGINS_RESOURCE_NAME)
    ...
    def _instantiate_selected(self, selection: dict[str, dict[str, Any]], *, config_hash: str) -> dict[str, Any]:
        ...
        for name in ordered_names:
            ...
            kind_cfg = next(
                (cfg for kind, cfg in selection.items() if cfg.get("name") == name and kind == spec.kind), None
            )
