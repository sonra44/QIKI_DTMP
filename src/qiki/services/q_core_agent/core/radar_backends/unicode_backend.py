...
class UnicodeRadarBackend(RadarBackend):
    ...
    def _render_grid(
        ...
        *,
        ...
        for situation in situations:
            ...
            severity = str(
                getattr(getattr(situation, "severity", ""), "value", getattr(situation, "severity", ""))
            ).upper()
            if is_acked:
        ...
        for point_id, u, v, depth, vr_mps in points:
            ...
            if (
                view_state.selected_target_id
                and point_id == view_state.selected_target_id
                and (render_plan is None or render_plan.draw_selection_highlight)
            ):
                marker = "◆"
        ...
        if view_state.alerts.situations_enabled:
            for situation in situations:
                situation_type = str(
                    getattr(getattr(situation, "type", ""), "value", getattr(situation, "type", ""))
                ).upper()
                severity = str(
                    getattr(getattr(situation, "severity", ""), "value", getattr(situation, "severity", ""))
                ).upper()
    ...
    def _projected_points(
        points: list, *, view_state: RadarViewState
