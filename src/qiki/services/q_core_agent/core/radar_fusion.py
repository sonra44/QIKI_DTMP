    ...
    for source_id in sorted(tracks_by_source.keys()):
        ...
        for track in tracks:
            ...
            contributor = Contributor(
                ...
                vel_xy=(
                    None if track.state_vel_xy is None else (float(track.state_vel_xy[0]), float(track.state_vel_xy[1]))
                ),
                dt=dt,
...
def associate(
    ...
    for seed in ordered:
        ...
        for source_id in sorted({candidate.source_id for candidate in ordered}):
            ...
            nearest = min(
                ...
                key=lambda candidate: (
                    _dist(seed.pos_xy, candidate.pos_xy),
                    -candidate.trust,
        ...
        contributors = tuple(sorted(selected, key=lambda c: (c.source_id, c.source_track_id)))
        clusters.append(
            FusionCluster(
...
def update_fusion_state(
    ...
    next_cooldown = {key: until_ts for key, until_ts in cooldown_prev.items() if until_ts > now}
