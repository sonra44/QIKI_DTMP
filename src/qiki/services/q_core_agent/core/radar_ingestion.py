...
def observation_to_source_track(observation: Observation) -> SourceTrack:
    ...
    return SourceTrack(
        ...
        state_vel_xy=(
            None if observation.vel_xy is None else (float(observation.vel_xy[0]), float(observation.vel_xy[1]))
        ),
        quality=quality,
        trust=quality,
        ...
        if event_store is not None and emit_observation_rx:
            event_store.append_new(
                ...
                payload={
                    ...
                    "vel": (None if track.state_vel_xy is None else [track.state_vel_xy[0], track.state_vel_xy[1]]),
