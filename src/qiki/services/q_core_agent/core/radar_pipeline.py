...
class RadarPipeline:
    ...
    def _effective_policy(self) -> RadarRenderPolicy:
        return self.policy_plugin.effective_policy(
            base_policy=self.render_policy, adaptive_level=self._adaptive_state.level
        )
    ...
    def _update_adaptive_state(self, *, frame_time_ms: float, targets_count: int) -> None:
        ...
        ema_frame = (
            float(frame_time_ms)
            if prev.ema_frame_ms is None
            else ((alpha_frame * float(frame_time_ms)) + ((1.0 - alpha_frame) * prev.ema_frame_ms))
        )
        ema_targets = (
            float(targets_count)
            if prev.ema_targets is None
            else ((alpha_targets * float(targets_count)) + ((1.0 - alpha_targets) * prev.ema_targets))
        )
        budget = float(self.render_policy.frame_budget_ms)
        target_budget = float(self.render_policy.clutter_targets_max)
        high = ema_frame > (budget * float(self.adaptive_policy.high_frame_ratio)) or ema_targets > (
            target_budget * float(self.adaptive_policy.overload_target_ratio)
        )
        low = ema_frame < (budget * float(self.adaptive_policy.low_frame_ratio)) and ema_targets < (
            target_budget * float(self.adaptive_policy.underload_target_ratio)
        )
        consecutive_high = (prev.consecutive_high + 1) if high else 0
        consecutive_low = (prev.consecutive_low + 1) if low else 0
        now = self._clock.now()
        elapsed_ms = (
            (now - prev.last_change_ts) * 1000.0
            if prev.last_change_ts > 0.0
            else float(self.adaptive_policy.cooldown_ms) + 1.0
        )
    ...
    def render_scene(self, scene: RadarScene, *, view_state: RadarViewState | None = None) -> RenderOutput:
        ...
        try:
            ...
            self._update_adaptive_state(
                frame_time_ms=self._last_frame_time_ms, targets_count=len(scene_with_trails.points)
            )
        ...
        *,
        ...
        for track in fused_tracks:
            signature = (
                ...
                tuple(
                    sorted(
                        f"{contributor.source_id}:{contributor.source_track_id}" for contributor in track.contributors
                    )
                ),
            )
            ...
            self.event_store.append_new(
                ...
                payload={
                    ...
                    "vel": (None if track.vel_xy is None else [float(track.vel_xy[0]), float(track.vel_xy[1])]),
