...
class SessionServer:
    ...
    def __init__(
        ...
        *,
        ...
        if self.auth_enabled and not self.session_token:
            raise RuntimeError(
                "QIKI_SESSION_AUTH=1 requires non-empty token from QIKI_SESSION_TOKEN or QIKI_SESSION_TOKEN_FILE"
            )
        allowed_raw = str(env.get("QIKI_SESSION_ALLOWED_ROLES", "viewer,controller,admin")).strip()
        self.allowed_roles = {part.strip().lower() for part in allowed_raw.split(",") if part.strip()} or {
            "viewer",
            "controller",
        }
    ...
    def _disconnect_client(self, client_id: str, *, reason: str = "CLIENT_DISCONNECTED") -> None:
        with self._clients_lock:
            client = self._clients.get(client_id)
        if client is None:
            return
        ...
        try:
            ...
            pass
        self._unregister_client(client_id, disconnect_reason=reason)
