    ...
    async def test_rapid_state_transitions(self, stress_store):
        ...
        with PerformanceMonitor("Rapid State Transitions") as monitor:
            ...
            for i in range(HIGH_LOAD_OPERATIONS):
                ...
                # Создаём переход
                next_snap = next_snapshot(current=current, new_state=new_state, reason=f"RAPID_TRANSITION_{i}")
...
class TestConcurrencyStress:
    ...
    async def test_subscriber_stress_with_backpressure(self, stress_store):
        ...
        async def producer():
            ...
            for i in range(100):
                snapshot = FsmSnapshotDTO(version=i + 2000, state=FsmState.ACTIVE, reason=f"BACKPRESSURE_{i}")
                await stress_store.set(snapshot)
            ...
            for i in range(500):
                try:
                    if random.random() < 0.1:  # 10% ошибок
                        ...
                        # Нормальная операция
                        good_snapshot = FsmSnapshotDTO(version=i, state=FsmState.IDLE, reason=f"NORMAL_OP_{i}")
