...
async def test_fsm_handler_initial_state_transition():
    ...
    initial_state = FsmSnapshotDTO(version=0, state=FsmState.BOOTING, reason="INIT", history=tuple())
...
async def test_fsm_handler_active_to_idle_on_no_proposals():
    ...
    fsm_handler = FSMHandler(context)
    initial_state = FsmSnapshotDTO(version=2, state=FsmState.ACTIVE, reason="ACTIVE", history=tuple())
...
async def test_fsm_handler_error_to_idle_on_recovery():
    ...
    fsm_handler = FSMHandler(context)
    initial_state = FsmSnapshotDTO(version=3, state=FsmState.ERROR_STATE, reason="ERROR", history=tuple())
