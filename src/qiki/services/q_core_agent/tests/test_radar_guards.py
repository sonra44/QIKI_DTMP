...
def test_world_model_builds_tracks_from_radar_frame_detections() -> None:
    guard_table = _default_guard_table()
    world_model = WorldModel(guard_table)
    ...
    snapshot = world_model.snapshot()
    assert snapshot["active_track_count"] == 1
    track = snapshot["radar_tracks"][0]
    assert track["transponder_id"] == "ALLY-652D53"
    assert track["range_m"] == 42.0
    assert track["quality"] == 1.0


def test_world_model_keeps_frame_track_identity_across_transponder_and_sensor_mutation() -> None:
    guard_table = _default_guard_table()
    world_model = WorldModel(guard_table)
    initial_frame = RadarFrameModel(
        sensor_id=uuid4(),
        detections=[
            RadarDetectionModel(
                range_m=4200.0,
    ...
    mutated_frame = RadarFrameModel(
        ...
        detections=[
            RadarDetectionModel(
                ...
                transponder_id="SPOOF-652D53",
            )
        ],
    )

    world_model.ingest_sensor_data(
        SensorData(sensor_id="radar_primary", sensor_type=SensorTypeEnum.RADAR, radar_frame=initial_frame)
    )
    initial_track = world_model.snapshot()["radar_tracks"][0]

    world_model.ingest_sensor_data(
        SensorData(sensor_id="radar_secondary", sensor_type=SensorTypeEnum.RADAR, radar_frame=mutated_frame)
    )
    mutated_snapshot = world_model.snapshot()

    assert mutated_snapshot["active_track_count"] == 1
    mutated_track = mutated_snapshot["radar_tracks"][0]
    assert mutated_track["object_identity"] == str(initial_track["track_id"])
    assert mutated_track["track_id"] == initial_track["track_id"]
    assert initial_track["visible_signature"] == "ALLY-652D53"
    assert mutated_track["visible_signature"] == "SPOOF-652D53"
    assert mutated_track["transponder_id"] == "SPOOF-652D53"
    assert mutated_track["transponder_mode"] == TransponderModeEnum.SPOOF
...
async def test_fsm_handler_transitions_on_critical_guard():
    ...
    initial_state = FsmSnapshotDTO(version=1, state=FsmState.IDLE, reason="IDLE", history=tuple())
