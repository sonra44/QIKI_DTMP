...
def test_eventstore_contract_fields_present() -> None:
    ...
    pipeline = RadarPipeline(
        RadarRenderConfig(renderer="unicode", view="top", fps_max=10, color=False), event_store=store
    )
...
def test_integration_sequence_expected_event_order() -> None:
    ...
    pipeline.situation_engine = _engine(
        confirm_frames=2, lost_contact_window_s=0.02, auto_resolve_after_lost_s=0.03, cooldown_s=0.05
    )
