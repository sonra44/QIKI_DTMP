...
def test_auth_accepts_good_token(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("QIKI_SESSION_AUTH", "1")
    monkeypatch.setenv("QIKI_SESSION_TOKEN", "secret-token")
    store = EventStore(maxlen=2000, enabled=True)
    pipeline = RadarPipeline(event_store=store)
    ...
    try:
        assert _wait_until(
            lambda: any(event.event_type == "SESSION_CLIENT_CONNECTED" for event in store.filter(subsystem="SESSION"))
        )
        assert not any(err.get("code") == "auth_failed" for err in client.recent_errors())
        assert not any(event.event_type == "SESSION_CLIENT_AUTH_FAILED" for event in store.filter(subsystem="SESSION"))
    finally:
        client.close()
        server.stop()
        pipeline.close()
...
def test_role_downgrade_non_strict(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        assert _wait_until(lambda: bool(client.latest_snapshot()))
        role_events = [
            event for event in store.filter(subsystem="SESSION") if event.event_type == "SESSION_ROLE_DOWNGRADED"
        ]
        assert role_events
...
def test_rate_limit_strict_disconnect(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        disconnect_events = [
            event for event in store.filter(subsystem="SESSION") if event.event_type == "SESSION_CLIENT_DISCONNECTED"
        ]
