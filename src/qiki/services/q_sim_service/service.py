...
_QSIM_SENSOR_QUEUE_DEFAULT_MAX = 256
_QSIM_RADAR_FRAMES_DEFAULT_MAX = 256
_QSIM_OVERFLOW_LOG_PERIOD_S = 10.0
...
class QSimService:
    def __init__(self, config: QSimServiceConfig):
        ...
        self._overflow_log_last_mono: dict[str, float] = {}
        self._sensor_queue_max = self._read_queue_max(
            env_name="QSIM_SENSOR_QUEUE_MAX",
            default=_QSIM_SENSOR_QUEUE_DEFAULT_MAX,
        )
        self._radar_frames_max = self._read_queue_max(
            env_name="QSIM_RADAR_FRAMES_MAX",
            default=_QSIM_RADAR_FRAMES_DEFAULT_MAX,
        )
        self.sensor_data_queue: deque[SensorReading] = deque(maxlen=self._sensor_queue_max)
    ...
    def _read_queue_max(self, env_name: str, default: int) -> int:
        ...
        try:
            ...
            pass
        logger.warning("%s invalid=%r, fallback to default=%d", env_name, raw, default)
        return default

    def _append_sensor_data(self, sensor_data: SensorReading) -> None:
        # Canonical live contract: q-core consumes the freshest raw sample on demand.
        # Historical sensor replay does not use this queue; keeping stale entries here
        # only creates false backlog pressure that pollutes comms latency truth.
        if self.sensor_data_queue:
            self.sensor_data_queue.clear()
        self.sensor_data_queue.append(sensor_data)

    def _append_radar_frame(self, frame: RadarFrameModel) -> None:
        if len(self.radar_frames) == self.radar_frames.maxlen:
            self._log_overflow_throttled(
                key="radar_frames",
                message="QSIM radar_frames overflow: dropped oldest (max=%d)",
                maxlen=self.radar_frames.maxlen or self._radar_frames_max,
            )
        self.radar_frames.append(frame)

    def _log_overflow_throttled(self, *, key: str, message: str, maxlen: int) -> None:
        now = time.monotonic()
        last = self._overflow_log_last_mono.get(key, 0.0)
        if now - last < _QSIM_OVERFLOW_LOG_PERIOD_S:
            return
        self._overflow_log_last_mono[key] = now
        logger.warning(message, maxlen)
    ...
    def step(
        ...
        *,
        ...
        self._append_sensor_data(sensor_data)
    ...
    def _build_telemetry_payload(self, state: dict) -> dict:
        ...
        comms_enabled = bool(comms.get("enabled"))
        ...
        if not comms_enabled:
            ...
            data_rate_kbps = 0.0
            antenna_status = "unlock"
        elif not xpdr_active:
            link = "degraded"
            latency_ms = 900.0 + queue_penalty_ms
            ...
            link = "online"
            latency_ms = 80.0 + queue_penalty_ms + max(0.0, (35.0 - soc_for_link) * 3.0)
            packet_loss_pct = min(25.0, max(0.0, (20.0 - soc_for_link) * 0.25))
            rssi_dbm = max(-110.0, -58.0 - (100.0 - soc_for_link) * 0.35)
            snr_db = max(0.0, 28.0 - (100.0 - soc_for_link) * 0.2)
        ...
        comms.update(
            {
                ...
                "data_rate_kbps": float(data_rate_kbps),
        ...
        payload = TelemetrySnapshotModel(
            ...
            speed_m_s=float(state.get("speed_m_s", state.get("speed", 0.0))),
            velocity_xyz_m_s=state.get("velocity_xyz_m_s"),
