...
def test_control_response_payload_falls_back_to_message_id_when_no_correlation_id() -> None:
    cfg = QSimServiceConfig(sim_tick_interval=1, sim_sensor_type=1, log_level="INFO")
    qsim = QSimService(cfg)

    req_id = UUID("00000000-0000-0000-0000-00000000000a")
    meta = MessageMetadata(
        message_type="control_command",
        source="test",
        destination="q_sim_service",
    ...
    cmd = CommandMessage(command_name="sim.pause", parameters={}, metadata=meta)
    ...
    assert resp["requestId"] == str(req_id)
    assert resp["request_id"] == str(req_id)
    assert resp["success"] is resp["ok"] is True


def test_control_response_generic_rejection_has_structured_error_detail() -> None:
    cfg = QSimServiceConfig(sim_tick_interval=1, sim_sensor_type=1, log_level="INFO")
    qsim = QSimService(cfg)

    meta = MessageMetadata(message_type="control_command", source="test", destination="q_sim_service")
    cmd = CommandMessage(command_name="sim.unknown", parameters={}, metadata=meta)

    status, error = _describe_control_command_result(cmd, success=False, sim_service=qsim)
    assert (status, error) == ("rejected", "rejected")

    resp = _build_control_response_payload(cmd, success=False, status=status, error=error)
    assert resp["success"] is resp["ok"] is False
    assert resp["error"] == "rejected"
    assert resp["error_detail"]["code"] == "rejected"
    assert resp["error_detail"]["message"] == "rejected"
    assert resp["error_detail"]["details"]["command_name"] == "sim.unknown"
