...
def test_telemetry_payload_includes_3d_position() -> None:
    ...
    velocity_xyz = payload.get("velocity_xyz_m_s")
    assert isinstance(velocity_xyz, dict)
    assert set(velocity_xyz.keys()) >= {"x", "y", "z"}
    assert isinstance(payload.get("speed_m_s"), float)

    orbit = payload.get("orbit")
    assert isinstance(orbit, dict)
    assert orbit.get("state") in {"healthy", "degraded", "failed", "off"}
    assert isinstance(orbit.get("confidence"), float)
