...
def handle_chat_request(request: QikiChatRequestV1, *, current_mode: QikiMode) -> QikiChatResponseV1:
    ...
    # This alternate legacy-compatible ingress only acknowledges operator decisions
    # deterministically. In the supported canonical contour, execution ownership
    # lives upstream on q-core-intents (`qiki.intents -> qiki.responses.qiki`),
    # while other historical paths may still reuse this handler.
