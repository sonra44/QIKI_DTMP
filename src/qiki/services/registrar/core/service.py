"""Registrar-local audit append service.

This module owns only registrar-local logging / file append concerns. It does not own
runtime truth, control, or the shared project-wide event ledger.
"""
...
class RegistrarEvent:
    """Represents one registrar-observed or registrar-generated audit event."""
...
class RegistrarService:
    """Registrar-local audit sink for append-only support evidence."""
    ...
    def register_event(self, event: RegistrarEvent) -> None:
        """Register an event in registrar-local sinks."""
        ...
        # Shared audit fan-out is handled by the FastStream layer in main.py.
        # This class stays narrow: local append/log support only.
    ...
    def register_boot_event(self, boot_status: str, details: Dict[str, Any]) -> None:
        ...
        event = RegistrarEvent(
            ...
            payload={"status": boot_status, "details": details},
            severity="INFO",
        )
    ...
    def register_sensor_event(self, sensor_id: str, status: str, details: Dict[str, Any]) -> None:
        ...
        event = RegistrarEvent(
            ...
            timestamp=datetime.now(timezone.utc),
            payload={"sensor_id": sensor_id, "status": status, "details": details},
            severity="INFO",
        )
        self.register_event(event)
