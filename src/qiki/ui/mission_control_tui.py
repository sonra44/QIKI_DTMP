...
from textual.widgets import (
    Header, Footer, Static, ProgressBar, DataTable, 
    Input, Log, TabbedContent, TabPane
)
...
try:
    ...
    from qiki.services.q_core_agent.core.ship_actuators import (
        ShipActuatorController, ThrusterAxis, PropulsionMode
    )
...
class RadarWidget(Static):
    ...
    def render(self) -> Panel:
        ...
        for track in tracks:
            ...
            content.append(f"│   {tid}   │  {ttype:<4}  │   {track['range']:<4}   │    {track['bearing']:03d}    │    {track['velocity']:+4.1f}    │  {track['snr']:4.1f}  │  TRACK   │ {track['iff']:<7} │")
            content.append(f"│   {tid_ru}   │  {ttype_ru:<4}  │   {track['range']:<4}   │    {track['bearing']:03d}    │    {track['velocity']:+4.1f}    │  {track['snr']:4.1f}  │  СЛЕЖ    │ {iff_map[track['iff']]:<7} │")
        ...
        content.append("LR: ACTIVE    │ ДД: АКТИВЕН          SR: ACTIVE    │ БД: АКТИВЕН          RANGE: 5.0 km    │    TRACKS: 3/16")
        
        panel_content = "\n".join(content)
        return Panel(
            panel_content,
            title="RADAR CONTACTS | РАДАРНЫЕ ЦЕЛИ",
...
class PropulsionWidget(Static):
    ...
    def render(self) -> Panel:
        ...
        content.append(f"             │               {self.prop_data.get('main_drive_thrust_n', 0):.1f} N      {'ACTIVE' if thrust_pct > 0 else 'IDLE'}")
        content.append("")
        ...
        return Panel(
            panel_content,
            title="PROPULSION | ДВИГАТЕЛЬНАЯ УСТАНОВКА",
...
class PowerWidget(Static):
    ...
    def render(self) -> Panel:
        ...
        for consumer in consumers:
            ...
            content.append(f"{consumer['name']:<13} │  {consumer['name_ru']:<8}      {consumer['power']:.1f} MW  {consumer['pct']}%")
        ...
        return Panel(
            panel_content,
            title="POWER DISTRIBUTION | РАСПРЕДЕЛЕНИЕ ЭНЕРГИИ",
...
class EventLogWidget(Static):
    ...
    def render(self) -> Panel:
        ...
        for event in self.events[-6:]:  # Show last 6 events
            content.append(f"{event['time']}  [{event['level']}]   │  {event['message_en']:<35}  │  {event['message_ru']}")
        ...
        return Panel(
            panel_content,
            title="SYSTEM EVENTS | СИСТЕМНЫЕ СОБЫТИЯ",
            border_style="white"
        )
...
class CommandInputWidget(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static(f"LAST COMMAND | ПОСЛЕДНЯЯ КОМАНДА:  {self.last_command}        STATUS | СТАТУС:  {self.last_status}", id="cmd_status")
        yield Static("HOTKEYS | ГОРЯЧИЕ КЛАВИШИ:    F1-Help  F2-Radar  F3-Systems  F4-Power  ESC-Exit", id="cmd_hotkeys")
...
class QIKIMissionControlApp(App):
    ...
    def _initialize_qiki_components(self):
        ...
        try:
            q_core_agent_root = os.path.abspath(os.path.join(PROJECT_ROOT, "src", "qiki", "services", "q_core_agent"))
    ...
    def update_telemetry(self) -> None:
        ...
        try:
            ...
            system_data = {
                'hull': {
                    'integrity': hull_status.integrity,
                    'mass_kg': hull_status.mass_kg
                },
                'power': {
                ...
                'life_support': {
                    'oxygen_percent': life_status.atmosphere.get('oxygen_percent', 21.2)
                },
                'computing': {
            ...
            nav_data = {
                'pos_x': 1250.4, 'pos_y': -890.1, 'pos_z': 45.2,
                'vel_x': 2.1, 'vel_y': -0.5, 'vel_z': 0.0,
                'pitch': 2.0, 'roll': -1.0, 'yaw': 45.0, 'heading': 45,
