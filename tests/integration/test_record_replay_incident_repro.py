...
async def test_record_replay_reproduces_incident_trigger_event(tmp_path: Path) -> None:
    ...
    try:
        ...
        async def inject_overcurrent_event() -> None:
            await asyncio.sleep(0.25)
            await nc.publish(SIM_POWER_PDU, json.dumps(injected_payload).encode("utf-8"))
            await nc.flush(timeout=1.0)

        inject_task = asyncio.create_task(inject_overcurrent_event())
        ...
        if incident_trigger is None:
            pytest.fail("No deterministic overcurrent payload captured in record window")
