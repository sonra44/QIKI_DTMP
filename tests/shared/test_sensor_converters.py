...
def test_proto_to_pydantic_preserves_sensor_timestamp() -> None:
    proto = SensorReading(
        sensor_type=SensorTypeEnum.IMU.value,
    )
    proto.sensor_id.value = "00000000-0000-0000-0000-000000000123"
    proto.timestamp.FromDatetime(datetime(2026, 3, 29, 12, 34, 56, tzinfo=UTC))
    proto.vector_data.x = 1.0
    proto.vector_data.y = 2.0
    proto.vector_data.z = 3.0

    converted = proto_sensor_reading_to_pydantic_sensor_data(proto)

    assert converted.timestamp == datetime(2026, 3, 29, 12, 34, 56, tzinfo=UTC)


def test_proto_to_pydantic_missing_sensor_timestamp_defaults_to_epoch() -> None:
    proto = SensorReading(
        sensor_type=SensorTypeEnum.IMU.value,
    )
    proto.sensor_id.value = "00000000-0000-0000-0000-000000000123"
    ...
    assert converted.timestamp == datetime(1970, 1, 1, 0, 0, tzinfo=UTC)
