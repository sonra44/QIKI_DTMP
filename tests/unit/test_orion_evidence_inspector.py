...
def test_claims_for_subsystem_one_per_field() -> None:
    ...
    assert [c.claim_id for c in claims] == ["power.soc_pct", "power.supercap_soc_pct"]
    # missing supercap carries the §19.5 ORION_SOURCE_MISSING
    assert "ORION_SOURCE_MISSING" in claims[1].reason_codes
...
def test_format_subsystem_inspector_header_and_blocks() -> None:
    ...
    assert "power.soc_pct" in text and "power.supercap_soc_pct" in text
