...
def test_orion_mission_seed_row_has_clear_message() -> None:
    ...
    def _fake_query_one(selector, _cls=None):  # noqa: ANN001
        if selector == "#mission-table":
            return _FakeDataTable()
        raise LookupError(selector)

    app.query_one = _fake_query_one  # type: ignore[method-assign]

    # No mission/task events in snapshots -> seed_empty path.
    app._render_mission_table()

    assert "seed" in captured
    cells = captured["seed"]
    assert I18N.bidi("No mission/task data", "Нет данных миссии/задач") in str(cells[-1])
