...
def test_thermal_trip_core_sheds_nbl_next_tick() -> None:
    bot_config = {
        "hardware_profile": {
            "power_capacity_wh": 500.0,
            "power_plane": {
    ...
    wm.step(1.0)
    assert "THERMAL_TRIP:core" in wm.power_faults
...
def test_nbl_budget_reason_is_emitted_when_nbl_blocked_without_thermal_trip() -> None:
    bot_config = {
        "hardware_profile": {
            "power_capacity_wh": 500.0,
            "power_plane": {
    ...
    assert "thermal_overheat" not in wm.power_shed_reasons
...
def test_pdu_overcurrent_fault_is_reported_when_base_load_exceeds_limit() -> None:
    bot_config = {
        "hardware_profile": {
            "power_capacity_wh": 500.0,
            "power_plane": {
    ...
    assert "PDU_OVERCURRENT" in wm.power_faults
