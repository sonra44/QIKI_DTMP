...
async def _main() -> None:
    ...
    try:
        ...
        async with app.run_test(size=(160, 48)) as pilot:
            ...
            await app._on_qiki_response(_response_payload(qsim, 701))
            await pilot.pause()

            assert app._qiki_pending_action is not None
            assert app._qiki_pending_action["name"] == "hostile_rcs_intercept_burst"
            propulsion0 = (app._snapshot.get("propulsion") or {}).get("rcs") or {}
            prepared_prop = float(propulsion0.get("propellant_kg") or 0.0)
            ...
            app.action_select_subsystem("propulsion")
            await pilot.pause()

            await app._on_qiki_response(_response_payload(qsim, 702))
            await pilot.pause()

            propulsion1 = (app._snapshot.get("propulsion") or {}).get("rcs") or {}
            final_prop = float(propulsion1.get("propellant_kg") or 0.0)
            ...
            assert "Топливо" in systems_text
            assert "Осталось до исчерпания" in systems_text

            print("OK: orion_v_qiki_combat_system_consequence_smoke")
            print(f"PREPARED_PROP_KG={prepared_prop:.5f}")
            print(f"FINAL_PROP_KG={final_prop:.5f}")
            print(f"FOLLOWUP_CODE={response.legality.reason_code}")
            print(f"FOLLOWUP_STATUS={response.legality.status}")
    finally:
