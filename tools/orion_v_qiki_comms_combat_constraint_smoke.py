...
from qiki.shared.models.radar import TransponderModeEnum
...
def _world_snapshot(qsim: QSimService) -> dict:
    state = qsim._build_telemetry_payload(qsim.world_model.get_state())
    state["radar_tracks"] = [
        {
            "object_type": 2,
            "range_m": 1800.0,
...
async def _main() -> None:
    ...
    qsim.world_model._rcs_propellant_kg = 20.0
    qsim.world_model._max_bus_a = 60.0
    ...
    try:
        ...
        async with app.run_test(size=(160, 48)) as pilot:
            ...
            await app._on_qiki_response(_response_payload(qsim, 1001))
            await pilot.pause()
            ...
            qsim.transponder_mode = TransponderModeEnum.OFF
            await app._on_telemetry({"data": qsim._build_telemetry_payload(qsim.world_model.get_state())})
            await pilot.pause()

            app.action_show_level("f2")
            app.action_select_subsystem("comms")
            await pilot.pause()

            await app._on_qiki_response(_response_payload(qsim, 1002))
            await pilot.pause()
            ...
            comms = app._snapshot.get("comms") or {}

            assert comms.get("link_state") == "degraded"
            assert comms.get("antenna_status") == "unlock"
            assert float(comms.get("data_rate_kbps") or 0.0) == 0.0
            assert "Состояние канала:" in systems_text
            assert "Задержка:" in systems_text
            assert "data_rate:" in systems_text

            print("OK: orion_v_qiki_comms_combat_constraint_smoke")
            print(f"COMMS_CODE={response.legality.reason_code}")
            print(f"COMMS_STATUS={response.legality.status}")
            print(f"COMMS_LINK={comms.get('link_state')}")
            print(f"COMMS_RATE={float(comms.get('data_rate_kbps') or 0.0):.2f}")
            print(f"COMMS_ANTENNA={comms.get('antenna_status')}")
    finally:
