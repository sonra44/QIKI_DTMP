...
class QikiDocgenConfig:
    ...
    def _find_project_root(self) -> Path:
        ...
        current_dir = Path(
            __file__
        ).parent.parent.parent.parent  # Поднимаемся к QIKI_DTMP
                ...
                if indent_level == 0:
                    ...
                    if key.startswith("- "):
                        ...
                        if isinstance(current_dict, dict):
                            ...
                            last_key = (
                                list(current_dict.keys())[-1] if current_dict else None
                            )
                            if last_key and not isinstance(
                                current_dict[last_key], list
                            ):
                                current_dict[last_key] = []
                            if last_key:
                                current_dict[last_key].append(
                                    self._parse_value(list_key)
                                )
                    else:
                        current_dict[key] = self._parse_value(value)
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "protoc": {
                ...
                "options": {"generate_pyi": False, "package_relative": True},
            },
            "generation": {
