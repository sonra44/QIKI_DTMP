...
async def main() -> int:
    ...
    try:
        ...
        persisted_mission = await get_persisted(deadline_epoch=time.time() + timeout_s, expect_mode=QikiMode.MISSION)
        if persisted_mission is None:
            return 1

        # Restore baseline for operators.
        await set_mode(QikiMode.FACTORY)
        persisted_factory = await get_persisted(deadline_epoch=time.time() + timeout_s, expect_mode=QikiMode.FACTORY)
